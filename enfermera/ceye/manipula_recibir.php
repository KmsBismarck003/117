...
$sql2 = "SELECT * FROM stock_ceye WHERE item_id=$item_id";
...
$sql3 = "SELECT * FROM material_ceye i, item_type it WHERE material_id=$item_id and i.material_tipo=it.item_type_id";
...
while ($row_item = $result_item->fetch_assoc()) {
   ...
   $item_type_desc=$row_item['item_type_desc'];
    ...
    $entrada= mysqli_query($conexion, 'INSERT INTO entradas_ceye(item_id,entrada_qty,entrada_added,id_usua) 
   values ('.$item_id.','.$entradas.',"'.$fecha_actual.'",'.$id_usu.') ') or die('<p>Error al registrar</p><br>' . mysqli_error($conexion));
     ...
     if ($existe === "SI") {
         ...
         $sql1="UPDATE stock_ceye SET stock_qty=$total,stock_entradas=$totentra where stock_id=$stock_id";
         $result_stock=$conexion->query($sql1);
      }else{
    
         $stock= mysqli_query($conexion, 'INSERT INTO stock_ceye(item_id,stock_qty,stock_entradas,stock_added,id_usua) 
