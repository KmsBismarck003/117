...
$result=mysqli_query($conexion,'INSERT INTO hprog_quir (id_usua,id_atencion,fecha_registro,tipo_cir,fecha_sol,hora_sol,hb,ht,persona,tiempo_estimado,matye,ope_proyectada,est_transo,diag_preop,d_posto,ope_realizada,cirujano,anest,tipo_ane,tip_sangre) 
	VALUES('.$id_usua.','.$id_atencion.',"'.$fecha_actual.'","'.$tipo_cir.'","'.$fecha_sol.'","'.$hora_sol.'","'.$hb.'","'.$ht.'","'.$persona.'","'.$tiempo_estimado.'","'.$matye.'","'.$ope_proyectada.'","'.$est_transo.'","'.$diag_preop.'","'.$d_posto.'","'.$ope_realizada.'","'.$cirujano.'","'.$anest.'","'.$tipo_ane.'","'.$tip_san.'")')or die('<p>Error al registrar</p><br>' . mysqli_error($conexion));
