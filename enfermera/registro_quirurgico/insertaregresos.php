...
$fechahora = date("Y-m-d H:i a");
       $ingresar9i = mysqli_query($conexion, 'INSERT INTO egresos_quir (id_usua,id_atencion,fecha,soluciones,hora,volumen,fecha_registro) values (' . $id_usua . ' ,' . $id_atencion .',"' . $fecha . '","' . $soluciones . '","' . $hora . '","' . $volumen . '","' . $fechahora . '") ') or die('<p>Error al registrar</p><br>' . mysqli_error($conexion));
