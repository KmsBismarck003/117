...
/*************************** modifica existencias  **********************/  
    ...
    $sql2c = $conexion->query("SELECT * FROM stock_ceye where item_id=$item_id") or die('<p>Error al encontrar stock CEYE</p><br>' . mysqli_error($conexion));
        ...
        if ($existec === "SI") {
            ...
            $ingresar = "INSERT INTO stock_ceye (item_id,stock_qty,stock_salidas,stock_added,id_usua) VALUES('$item_id','$qty','$qty_salida','$fecha_actual','$id_usua')";
            
            echo mysqli_query($conexion,$ingresar);
        }
 
   /*
   $sqlalm = "INSERT INTO cart_recib(item_id,cart_qty,destino,cart_uniqid,almacen)VALUES
('$item_id','$qty','TOLUCA','$cart_uniquid','CEYE')";
...
/*************************** inserta medica_enf **********************/    
include "../../conexionbd.php";
$ingresar9 = "INSERT INTO medica_enf (id_atencion,fecha_mat,medicam_mat,unimed,id_usua,enf_fecha,tipo,cantidad,material_id,material)
values ('$id_atencion','$fecha_actual','$med_nom','$unimed','$id_usua','$cart_fecha','$tipo','$qty','$item_id','Si')";
...
/*************************** Inserta solicitar al almacen desde CEYE  **********************/   
    ...
    if ($material_id <> 1124) {
        ...
        $sqlalm = "INSERT INTO cart_recib(item_id,cart_qty,destino,cart_uniqid,almacen,confirma)VALUES
              ('$item_id','$qty','TOLUCA','$cart_uniquid','CEYE','SI')";
        echo mysqli_query($conexion_almacen,$sqlalm);
    }
}
