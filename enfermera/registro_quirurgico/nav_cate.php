...
<div class="container">
  <div class="row">
    ...
    <div>
        &nbsp;
    <a class="btn btn-outline-primary   btn-sm active" href="nav_cate.php">CATÉTERES</a>
    </div>
    <div>
        &nbsp;
      ...
      <th scope="row" colspan="1">  <select name="dispositivos" class="form-control" >
        <option value="">Seleccionar dispositivo</option>
         <option value="CATETER CENTRAL">CATÉTER CENTRAL</option>
          <option value="CATETER PEREIFERICO">CATÉTER PEREIFERICO</option>
...
<td colspan="1">
    <textarea name="cultivo" class="form-control">
    </textarea>
...
$fr = date("Y-m-d H:i");
   $ingresar4 = mysqli_query($conexion, 'INSERT INTO cate_enf_ter (id_atencion,id_usua,dispositivos,tipo,fecha_inst,instalo,dias_inst,fecha_cambio,cultivo,fecha_registro,tip) values ('.$id_atencion.',' . $id_usua . ',"' . $dispositivos .'","' . $tipo . '","'.$fecha_inst.'","'.$instalo.'","'.$dias_inst.'","'.$fecha_cambio.'","'.$cultivo.'","'.$fr.'","Quirofano") ') or die('<p>Error al registrar</p><br>' . mysqli_error($conexion));
      ...
      echo '<script type="text/javascript">window.location.href = "nav_cate.php";</script>';
    }
    ?>
...
include "../../conexionbd.php";
$id_atencion=$_SESSION['pac'];
$resultado = $conexion->query("SELECT * from cate_enf_ter WHERE id_atencion=$id_atencion and tip='Quirofano' ORDER BY id_cate_ter DESC") or die($conexion->error);
$usuario = $_SESSION['login'];
?>
...
<table class="table table-bordered table-striped" id="mytable3">
       <thead class="thead">
         <tr class="table-primary">
            <th colspan="9"><center><h5><strong>CONTROL DE CATÉTERES</strong></h5></center></th>
         </tr>
...
<td><a href="edit_cate.php?id_cate_ter=<?php echo $f['id_cate_ter'];?>&id_atencion=<?php echo $f['id_atencion'];?>"><button type="button" class="btn btn-warning"><i class="fas fa-edit"></i></button></td>

<td><a href="el_cate.php?id_cate_ter=<?php echo $f['id_cate_ter'];?>&id_atencion=<?php echo $f['id_atencion'];?>"><button type="button" class="btn btn-danger">X </button></td>
