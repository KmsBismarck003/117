...
/*
  $sql2 = "SELECT * FROM stock_rojo4 where stock_id = $cart_stock_id";
...
*/
  ...
  $sql1 = "DELETE FROM cart_rojo4 WHERE cart_id = $cart_id";
  $result1 = $conexion->query($sql1);

  $sql3 = "DELETE FROM medicamentos_rojo4 WHERE cart_id = $cart_id";
  $result3 = $conexion->query($sql3);
/*
  $sql2 = "UPDATE stock_rojo4 set stock_qty=$stock_final, stock_salidas=$stock_salidas where stock_id = $cart_stock_id";
...
if (@$_GET['q'] == 'comf_cart') 
{
  ...
  $sql2 = "SELECT * FROM cart_rojo4 , material_rojo4 , item_type  where cart_rojo4.material_id = material_rojo4.material_id and material_rojo4.material_tipo=item_type.item_type_id and paciente = $id_atencion";
  $result = $conexion->query($sql2);
...
$fecha_actual = date("Y-m-d H:i:s");
    ...
    $existenciasc=0;
    $salidasc=0;
    $sql2c = $conexion->query("SELECT * FROM stock_rojo4 where stock_rojo4.item_id=$item_id") or die('<p>Error al encontrar stock rojo4</p><br>' . mysqli_error($conexion));
        ...
        if ($existec === "SI") {
            ...
            $sql3c = "UPDATE stock_rojo4 set stock_qty=$stock_finalc, stock_salidas=$salidas_finalc, stock_added='$fecha_actual',id_usua=$id_usua where stock_rojo4.item_id = $item_id";
            $result3c = $conexion->query($sql3c);
        }
        else {
            $ingresar = mysqli_query($conexion, 'insert into stock_rojo4 (item_id,stock_qty,stock_salidas,stock_added,id_usua) values(' . $item_id . ',' . $qty . ',' . $qty_salida . ',"' . $fecha_actual . '",' . $id_usua . ')') or die('<p>Error al registrar stock carro rojo</p><br>' . mysqli_error($conexion));
        }

  }
/*
 $insertar=mysqli_query($conexion,'INSERT INTO sales_rojo4(item_code,generic_name,brand,gram,type,qty,price, paciente) values ("'.$material_id.'","'.$material_nombre.'","'.$material_fabricante.'","'.$material_contenido.'","'.$material_tipo.'","'.$cart_qty.'","'.$material_precio.'","'.$paciente.'")')  or die('<p>Error al registrar</p><br>' . mysqli_error($conexion));
*/
  $sql_delCart = "DELETE FROM cart_rojo4 WHERE paciente = $id_atencion";
  $result_delCart = $conexion->query($sql_delCart);

  

  if ($result_delCart && $result_insert) {
header('location: nav_med_rojo4.php');
    /*echo '<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/sweetalert/1.1.3/sweetalert.css">';
    echo '<script src="https://code.jquery.com/jquery-2.1.3.min.js"></script>';
    echo '<script src="https://cdnjs.cloudflare.com/ajax/libs/sweetalert/1.1.3/sweetalert-dev.js"></script>';
    echo '<script>
