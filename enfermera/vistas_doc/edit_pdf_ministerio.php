...
<div class="thead" style="background-color: #2b2d7f; color: white; font-size: 20px;"><strong><center>NOTIFICACIÓN A MINISTERIO PÚBLICO </center> </strong></div>
                        <p>
                    </div>
...
<div class="row">
    <div class="col">
        <hr>
        <center><label><strong>Acto notificado: </strong></center>
            <div class="botones">
       ...
       <button type="button" class="btn btn-primary btn-sm" id="btnStopRecord"><i class="fas fa-microphone-slash"></button></i>
            ...
            <textarea name="acto" class="form-control" rows="4" id="texto"></textarea>   
...
<p></p>
        ...
        <label><strong>No. de reporte: </strong></label>
<input type="text" name="reporte" class="form-control">

<p></p>
        <strong>Agencia del ministerio público a la que notifica:</strong>
        <div class="botones">
        <button type="button" class="btn btn-danger btn-sm" id="btnagencia"><i class="fas fa-microphone">
</button></i>
       <button type="button" class="btn btn-primary btn-sm" id="stopage"><i class="fas fa-microphone-slash"></button></i>
      </div>
        <strong>
<textarea class="form-control" name="agencia" id="txtagencia"></textarea>
<script type="text/javascript">
const btnagencia = document.getElementById('btnagencia');
const stopage = document.getElementById('stopage');
const txtagencia = document.getElementById('txtagencia');
     ...
     let rea = new webkitSpeechRecognition();
      ...
      stopage.addEventListener('click', () => {
        rea.abort();
      });
</script>
<p>
        <p>Persona que recibe el reporte: 
        <p><input type="text" name="persona" class="form-control"></p>
...
<hr>
 ...
 <br>        
    ...
    <?php
      ...
      $acto =  mysqli_real_escape_string($conexion, (strip_tags($_POST["acto"], ENT_QUOTES)));
      $reporte =  mysqli_real_escape_string($conexion, (strip_tags($_POST["reporte"], ENT_QUOTES)));
      $agencia =  mysqli_real_escape_string($conexion, (strip_tags($_POST["agencia"], ENT_QUOTES)));
      $persona =  mysqli_real_escape_string($conexion, (strip_tags($_POST["persona"], ENT_QUOTES)));
...
$fechahora = date("Y-m-d H:i:s");
       $ingresar9 = mysqli_query($conexion, 'INSERT INTO mp (id_atencion,id_usua,fecha_mp,acto,reporte,agencia,persona) values (' . $id_atencion . ' ,' . $id_usua .',"' . $fecha_actual . '","' . $acto . '","' . $reporte . '","' . $agencia . '","' . $persona . '") ') or die('<p>Error al registrar</p><br>' . mysqli_error($conexion));

echo '<script >window.open("pdf_consent_ministerio.php?id_atencion='.$id_atencion.'&id_exp='.$id_exp.'&acto='.$acto.'&reporte='.$reporte.'&agencia='.$agencia.'&persona='.$persona.' ", "Notificación ministerio publico", "width=1000, height=1000")</script>'.
 '<script type="text/javascript">window.location.href ="edit_pdf_ministerio.php" ;</script>';
