      ...
      <div class="container-fluid">
          ...
          <div>
            <div class="thead" style="background-color: #2b2d7f; color: white; font-size: 20px;">
                <tr><strong><center>EDITAR GLICEMIA CAPILAR</center></strong>
            </div>
            <br>
            <?php
            $id = $_GET['id_mon_s'];

            $sql = "SELECT * from glic_ca where id_glc = $id";
...
<div class="col-sm-2">
  <label>Hora:</label>
<input type="time" class="form-control" name="hora_g" style="width : 100%; heigth : 100%" value="<?php echo $row_datos['hora_g']; ?>" required="">
</div>
    <div class="col-sm-3">
   <div class="form-group">
                  <label for="cantidad">Valor:</label>
                   <input type="text" size="30" name="valor"  id="cantidad" class="form-control" 
                  value="<?php echo $row_datos['valor']; ?>" required>
                </div>
    </div>
  </div>
</div>
...
$fecha_reporte    = mysqli_real_escape_string($conexion, (strip_tags($_POST["fecha_reporte"], ENT_QUOTES))); 
        ...
        $sql2 = "UPDATE glic_ca SET fecha_reporte = '$fecha_reporte',hora_g='$hora_g', valor = '$valor' WHERE id_glc = $id";
