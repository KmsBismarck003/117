...
$pdf->Cell(30, 11, utf8_decode('MÉDICO: '.$pre.'. '.$nom_usua.' '.$papell_usua.' '.$sapell_usua),0, 'L');
$pdf->Ln(11);
$pdf->Cell(30, 11, utf8_decode($anio),0, 'L');
$pdf->Ln(11);

$pdf->Cell(10, 11, utf8_decode('#'), 1, 'L');
$pdf->Cell(90, 11, utf8_decode('DIAGNÓSTICOS'), 1, 'L');
$pdf->Cell(90, 11, utf8_decode('CANTIDAD'), 1, 'L');
$pdf->Ln(11);
$pdf->SetFont('Arial', '', 10);
$sql_tabla = "SELECT motivo_atn, COUNT(motivo_atn) as cuantos FROM `dat_ingreso` WHERE YEAR(fecha)=$anio and id_usua=$id_usua GROUP BY 1 HAVING COUNT(motivo_atn)>=1 ORDER BY cuantos DESC";
