...
$pdf->SetX(25);
$pdf->Cell(150, 5, utf8_decode('REPORTE ADMINISTRATIVO ANUAL'), 1, 0, 'C');
$pdf->Ln(11);
...
$sql_tabla = "SELECT di.*,p.*,df.*,COUNT(df.id_atencion) as cuantos, SUM(deposito) as total FROM dat_financieros df,dat_ingreso di, paciente p WHERE p.Id_exp=di.Id_exp and di.id_atencion=df.id_atencion and YEAR(df.fecha)=$anio GROUP BY 1 HAVING COUNT(df.id_atencion)>=1 ORDER BY total DESC";
