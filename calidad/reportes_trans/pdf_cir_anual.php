...
$pdf->SetX(25);
$pdf->Cell(150, 5, utf8_decode('REPORTE DE CIRUGÍAS ANUAL'), 1, 0, 'C');
$pdf->Ln(11);
...
$pdf->Cell(140, 11, utf8_decode('DIAGNÓSTICO'), 1, 'L');
$pdf->Cell(40, 11, utf8_decode('CANTIDAD'), 1, 'L');
$pdf->Ln(11);
$pdf->SetFont('Arial', '', 10);
$sql_tabla = "SELECT diag_postop, COUNT(diag_postop) as cuantos FROM `dat_not_inquir` WHERE YEAR(fecha)=$anio GROUP BY 1 HAVING COUNT(diag_postop)>=1 ORDER BY cuantos DESC";
  $result_tabla = $conexion->query($sql_tabla);
  $no=1;
 while ($row_tabla = $result_tabla->fetch_assoc()) {
      $pdf->Cell(10, 11, utf8_decode($no), 1, 'L');
      $pdf->Cell(140, 11, utf8_decode($row_tabla['diag_postop']), 1, 'L');
      $pdf->Cell(40, 11, utf8_decode($row_tabla['cuantos']), 1, 'L');
      $pdf->Ln(11);
      $no++;
    }
