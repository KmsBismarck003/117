...
$pdf->Ln(7);
$pdf->SetFont('Arial', '', 8);
$sql_tabla = "SELECT id_usua, COUNT(id_usua) as cuantos FROM `dat_not_inquir` WHERE YEAR(fecha)=$anio and id_usua=$id_usua GROUP BY 1 HAVING COUNT(id_usua)>=1 ORDER BY cuantos DESC";
 ...
 while ($row_tabla = $result_tabla->fetch_assoc()) {
   ...
   $sql = "SELECT DATEDIFF(fecha,fec_egreso)* -1 as estancia FROM dat_ingreso WHERE YEAR(fecha)=$anio and id_usua=$id_usua";
...
$pdf->Ln(7);
$pdf->SetFont('Arial', '', 8);
$sql_tabla = "SELECT diag_postop, COUNT(diag_postop) as cuantos FROM `dat_not_inquir` WHERE YEAR(fecha)=$anio and id_usua=$id_usua GROUP BY 1 HAVING COUNT(diag_postop)>=1 ORDER BY cuantos DESC";
  $result_tabla = $conexion->query($sql_tabla);
  $no=1;
  $tot_cir=0;
 while ($row_tabla = $result_tabla->fetch_assoc()) {
...
$pdf->Ln(7);
$pdf->SetFont('Arial', '', 8);
$sql_tabla = "SELECT motivo_atn, COUNT(motivo_atn) as cuantos, fecha as fecha_ing, fec_egreso FROM `dat_ingreso` WHERE YEAR(fecha)=$anio and id_usua=$id_usua GROUP BY 1 HAVING COUNT(motivo_atn)>=1 ORDER BY cuantos DESC";
  $result_tabla = $conexion->query($sql_tabla);
  $no=1;
  $tot_ing=0;
 while ($row_tabla = $result_tabla->fetch_assoc()) {
