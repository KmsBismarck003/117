...
if (!$row_exp = $result_exp->fetch_assoc()) {
    ...
    header("Location: examenes_gab.php?error=" . urlencode("Atención no encontrada."));
    exit();
}
...
// Get the doctor's name and ID from the session
...
if (!$result_prices) {
    ...
    header("Location: examenes_gab.php?error=" . urlencode("Error al cargar servicios."));
    exit();
}
$prices = [];
$service_names = [];
...
if (empty($selected_services) && empty($otros_gabinete)) {
    ...
    header("Location: examenes_gab.php?error=" . urlencode("Seleccione al menos un servicio o especifique un estudio personalizado."));
    exit();
}
...
if (!empty($selected_services) || $otros_gabinete) {
    ...
    $sql_insert = "INSERT INTO ocular_examenes_gabinete (id_atencion, Id_exp, id_serv, otros_gabinete, precio_total) VALUES (?, ?, ?, ?, ?)";
    $stmt_insert = $conexion->prepare($sql_insert);
    if (!$stmt_insert) {
        error_log("Prepare failed for ocular_examenes_gabinete: " . $conexion->error);
        header("Location: examenes_gab.php?error=" . urlencode("Error preparando la consulta de datos."));
        exit();
    }
    ...
    if (!$stmt_ctapac) {
        ...
        header("Location: examenes_gab.php?error=" . urlencode("Error preparando la consulta de cuenta."));
        exit();
    }
    ...
    foreach ($selected_services as $serv_id => $value) {
        if ($value == 1 && isset($prices[$serv_id])) {
            ...
            $stmt_insert->bind_param("iiisd", $id_atencion, $Id_exp, $serv_id, $service_desc, $prices[$serv_id]);
    ...
    if ($otros_gabinete && $success) {
        ...
        $stmt_insert->bind_param("iissd", $id_atencion, $Id_exp, $custom_serv_id, $otros_gabinete, $custom_price);
        if (!$stmt_insert->execute()) {
            $success = false;
...
if ($success) {
    ...
    if (!$row_pac = $result_pac->fetch_assoc()) {
        ...
        header("Location: examenes_gab.php?error=" . urlencode("Paciente no encontrado."));
        exit();
    }
    ...
    $pdf->Cell(0, 5, utf8_decode("Detalle de "), 0, 1, 'L');
    $pdf->Cell(0, 5, utf8_decode("Diagnóstico probable: "), 0, 1, 'L');
    ...
    if (!$stmt_gab) {
        ...
        header("Location: examenes_gab.php?error=" . urlencode("Error preparando notificación."));
        exit();
    }
    $activo = 'SI';
    $realizado = 'NO';
    ...
    if ($stmt_gab->execute()) {
        if ($pdf->Output('F', $nombreFinal) === false) {
            error_log("Failed to generate PDF: $nombreFinal");
            header("Location: examenes_gab.php?error=" . urlencode("Error al generar PDF."));
            exit();
        }

        /* $_SESSION['mensaje'] = "Solicitud registrada y PDF generados exitosamente.";
        error_log("PDF generated successfully: $nombreFinal"); */
    ...
    header("Location: examenes_gab.php?error=" . urlencode("Error al registrar."));
    exit();
}
