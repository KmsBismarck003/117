...
$id_tratamiento_laser = isset($_GET['id_tratamiento_laser']) ? (int)$_GET['id_tratamiento_laser'] : 0;
$id_exp = isset($_GET['id_exp']) ? (int)$_GET['id_exp'] : 0;
$id_atencion = isset($_GET['id_atencion']) ? (int)$_GET['id_atencion'] : 0;

if ($id_tratamiento_laser <= 0 || $id_exp <= 0 || $id_atencion <= 0) {
    die("ID de tratamiento láser, expediente o atención no válido.");
}

// Fetch laser treatment data
$sql_laser = "SELECT * FROM ocular_tratamiento_laser WHERE id_tratamiento_laser = ? AND id_atencion = ? AND Id_exp = ?";
$stmt_laser = $conexion->prepare($sql_laser);
if (!$stmt_laser) {
    die("Error preparando consulta de tratamiento láser: " . $conexion->error);
}
$stmt_laser->bind_param("iii", $id_tratamiento_laser, $id_atencion, $id_exp);
$stmt_laser->execute();
$result_laser = $stmt_laser->get_result();
if (!$laser = $result_laser->fetch_assoc()) {
    die("Tratamiento láser no encontrado.");
}
$stmt_laser->close();
...
$stmt_doc->bind_param("i", $laser['id_usua']);
$stmt_doc->execute();
...
class PDF extends FPDF {
    function Header() {
        ...
        $this->Cell(0, 12, utf8_decode('NOTA DE TRATAMIENTO LÁSER'), 0, 1, 'C');
...
// Laser Studies Section
$pdf->SetFont('Arial', 'B', 12);
$pdf->SetFillColor(220, 230, 250);
$pdf->Cell(0, 10, utf8_decode('TRATAMIENTO LÁSER'), 0, 1, 'C', true);
$pdf->Ln(5);

$pdf->SetFont('Arial', 'B', 10);
$pdf->SetFillColor(245, 245, 245);
$pdf->Cell(35, 8, '', 1, 0, 'C', true);
$pdf->Cell(75, 8, utf8_decode('Ojo Derecho'), 1, 0, 'C', true);
$pdf->Cell(75, 8, utf8_decode('Ojo Izquierdo'), 1, 1, 'C', true);

$left_eye_fields = [
    'Tipo de láser' => ($laser['tipo_laser_izquierdo'] && $laser['tipo_laser_izquierdo'] !== 'Selecciona') ? $laser['tipo_laser_izquierdo'] : 'N/A',
    'Detalles' => $laser['detalles_laser_izquierdo'] ?? 'N/A',
];

$right_eye_fields = [
    'Tipo de láser' => ($laser['tipo_laser_derecho'] && $laser['tipo_laser_derecho'] !== 'Selecciona') ? $laser['tipo_laser_derecho'] : 'N/A',
    'Detalles' => $laser['detalles_laser_derecho'] ?? 'N/A',
];
...
foreach ($left_eye_fields as $label => $left_value) {
    ...
    if ($left_value !== 'N/A' || $right_value !== 'N/A') {
        ...
        $pdf->Cell(75, 5, utf8_decode($right_value), 1, 0, 'C');
        $pdf->Cell(75, 5, utf8_decode($left_value), 1, 1, 'C');
    }
}

$pdf->Ln(5);

/* // General Laser Fields
$general_fields = [
    'Indicaciones' => $laser['indicaciones_laser'] ?? 'N/A',
];
...
foreach ($general_fields as $label => $value) {
    if ($value !== 'N/A') {
        $pdf->MultiCell(0, 8, utf8_decode($label . ': ' . $value), 1, 'J', false);
    }
} */
...
header('Content-Disposition: inline; filename="tratamiento_laser_ocular.pdf"');
$pdf->Output('I', 'tratamiento_laser_ocular.pdf');
exit();
?>