...
$sql_obs = "SELECT * FROM dat_ob  where id_atencion = $id_atencion";
$result_obs = $conexion->query($sql_obs);
 ...
 echo '<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/sweetalert/1.1.3/sweetalert.css">';
    ...
    echo '<script>
                    $(document).ready(function() {
                        swal({
                            title: "No existe nota de observación para este paciente", 
                            type: "error",
...
$this->Ln(32);
  ...
  function Footer()
  {
    ...
    $this->Cell(0, 10, utf8_decode('MAC-4.04'), 0, 1, 'R');
  }
...
$sql_obs = "SELECT * FROM dat_ob  where id_ob = $id_ob";
$result_obs = $conexion->query($sql_obs);

while ($row_obs = $result_obs->fetch_assoc()) {
  $fecha_ob = $row_obs['fecha_ob'];
  //$tip_evo = $row_obs['tip_evo'];
  $problemao = $row_obs['problemao'];  
  $subjetivob = $row_obs['subjetivob'];  
  $objetivob = $row_obs['objetivob'];  
  $analisiso = $row_obs['analisiso']; 
  $trat_noturgen = $row_obs['trat_noturgen'];
  $guia = $row_obs['guia']; 
  $plano = $row_obs['plano']; 
  $pxo = $row_obs['pxo'];  
  $dest_cu_ob = $row_obs['dest_cu_ob'];  
...
$pdf->MultiCell(165, 9.5, utf8_decode('NOTA DE OBSERVACIÓN'), 0, 'C');
...
$dateo=date_create($fecha_ob);

$pdf->SetFont('Arial', '', 6);
$pdf->Cell(35, -2, 'Fecha: '.date_format($dateo,'d/m/Y H:i:s'), 0, 1, 'L');
...
$pdf->SetFont('Arial', 'B', 8);
$pdf->Cell(37, 3, utf8_decode('Motivo de atención: '), 0, 'L'); 
$pdf->SetFont('Arial', '', 8);
$pdf->MultiCell(158, 3, utf8_decode($problemao), 1, 'J');
$pdf->Ln(1);

$pdf->SetFont('Arial', 'B', 8);
$pdf->Cell(37, 3, utf8_decode('Resumen interrogatorio: '), 0, 'L'); 
$pdf->SetFont('Arial', '', 8);
$pdf->MultiCell(158, 3, utf8_decode($subjetivob), 1, 'J');
$pdf->Ln(1);

$pdf->SetFont('Arial', 'B', 8);
$pdf->Cell(37, 3, utf8_decode('Resultados relevantes: '), 0, 'L'); 
$pdf->SetFont('Arial', '', 8);
$pdf->MultiCell(158, 3, utf8_decode($objetivob), 1, 'J');
$pdf->Ln(1);

$pdf->SetFont('Arial', 'B', 8);
$pdf->Cell(37, 3, utf8_decode('Diagnóstico: '), 0, 'L'); 
$pdf->SetFont('Arial', '', 8);
$pdf->MultiCell(158, 3, utf8_decode($analisiso), 1, 'J');
$pdf->Ln(1);

$pdf->SetFont('Arial', 'B', 8);
$pdf->Cell(37, 3, utf8_decode('Tratamiento y plan: '), 0, 'L'); 
$pdf->SetFont('Arial', '', 8);
$pdf->MultiCell(158, 3, utf8_decode($trat_noturgen), 1, 'J');
$pdf->Ln(1);

$pdf->SetFont('Arial', 'B', 8);
$pdf->Cell(37, 3, utf8_decode('Pronóstico:'), 0, 'L'); 
$pdf->SetFont('Arial', '', 8);
$pdf->MultiCell(158, 3, utf8_decode($plano), 1, 'J');
$pdf->Ln(1);

$pdf->SetFont('Arial', 'B', 8);
$pdf->Cell(37, 3, utf8_decode('Observaciones y análisis:'), 0, 'L'); 
$pdf->SetFont('Arial', '', 8);
$pdf->MultiCell(158, 3, utf8_decode($pxo), 1, 'J');
$pdf->Ln(1);

$pdf->SetFont('Arial', 'B', 8);
$pdf->Cell(37, 3, utf8_decode('Guía práctica clínica:'), 0, 'L'); 
$pdf->SetFont('Arial', '', 8);
$pdf->MultiCell(158, 3, utf8_decode($guia), 1, 'J');
$pdf->Ln(1);

$pdf->SetFont('Arial', 'B', 8);
$pdf->Cell(37, 3, utf8_decode('Destino:'), 0, 'L'); 
$pdf->SetFont('Arial', '', 8);
$pdf->MultiCell(158, 3.1, utf8_decode($dest_cu_ob), 1, 'J');
$pdf->Ln(1);

$sql_med_id = "SELECT id_usua FROM dat_ob WHERE id_atencion = $id_atencion ORDER by fecha_ob DESC LIMIT 1";
...
$cargp = $row_med['cargp'];
      ...
      $pdf->SetFont('Arial', 'B', 8);
      //$pdf->Image('../../imgfirma/' . $firma, 96, 244, 25);
       if ($firma==null) {
