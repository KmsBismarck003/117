  ...
  $sql_dat_eg = "SELECT * from defuncion where id_atencion = $id_atencion";
  $result_dat_eg = $conexion->query($sql_dat_eg);
  

  while ($row_dat_eg = $result_dat_eg->fetch_assoc()) {
    $id_def = $row_dat_eg['id_def'];
  }
  if(isset($id_def)){
      $id_def = $id_def;
    }else{
    ...
    echo '<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/sweetalert/1.1.3/sweetalert.css">';
      ...
      echo '<script>
                      $(document).ready(function() {
                          swal({
                              title: "NO EXISTE NOTA DE DEFUNCION PARA ESTE PACIENTE", 
                              type: "error",
     ...
     function Footer()
  {
    ...
    $this->Cell(0, 10, utf8_decode('MAC-5.09'), 0, 1, 'R');
...
$pdf->MultiCell(165, 9.5, utf8_decode('NOTA DE DEFUNCIÓN'), 0, 'C');
...
$pdf->Ln(6);
 ...
 $sql_dat_eg = "SELECT * from defuncion where id_def=$id_in and id_atencion = $id_atencion";
  $result = $conexion->query($sql_dat_eg);

  while ($row_def = $result->fetch_assoc()) {
    $nota_defuncion=$row_def['defuncion'];
    $fech_egreso = $row_dat_eg['fecha'];
  }

$fecha_egreso=date_create($fech_egreso);
$pdf->SetFont('Arial', 'B', 8);
$pdf->Cell(25, 4, utf8_decode('Fecha de egreso: '),0, 'L');
$pdf->SetFont('Arial', '', 8);
$pdf->Cell(28, 4, utf8_decode(date_format($fecha_egreso,"d-m-Y H:i:s")),'B', 'L');
$pdf->Ln(6);
...
$pdf->Ln(1);
$pdf->SetFont('Arial', 'B', 8);
$pdf->Cell(195, 3, utf8_decode('Nota de defunción: '), 0,1, 'L');
$pdf->SetFont('Arial', '', 8);
$pdf->MultiCell(195, 3, utf8_decode($nota_defuncion),1, 'L');
$pdf->Ln(1);



  $sql_med_id = "SELECT id_usua FROM defuncion WHERE id_atencion = $id_atencion ORDER by id_def DESC LIMIT 1";
...
$pdf->Ln(20);
 ...
 $pdf->Image('../../imgfirma/FIRMA.jpg', 94, 240 , 30);
    ...
    $pdf->Image('../../imgfirma/' . $firma, 94, 240 , 30);
      ...
      $pdf->Cell(0, 4, utf8_decode('CMSI-5.09'), 0, 1, 'R');
