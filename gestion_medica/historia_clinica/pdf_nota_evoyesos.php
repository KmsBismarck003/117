...
//consulta yesos
$sql_ye = "SELECT yes.fecha_y,yes.tip_ey,yes.problemay,yes.subjetivoy,yes.objetivoy,yes.analisisy,yes.plany,yes.pxy,yes.dest_cu_yesos FROM dat_yesos yes, dat_ingreso din where din.id_atencion = $id_atencion";
$result_ye = $conexion->query($sql_ye);

while ($row_ye = $result_ye->fetch_assoc()) {
  $fecha_y = $row_ye['fecha_y'];
  $tip_ey = $row_ye['tip_ey'];
  $problemay = $row_ye['problemay'];  
  $subjetivoy = $row_ye['subjetivoy'];  
  $objetivoy = $row_ye['objetivoy'];  
  $analisisy = $row_ye['analisisy']; 
  $plany = $row_ye['plany']; 
  $pxy = $row_ye['pxy'];  
  $dest_cu_choque = $row_ye['dest_cu_yesos'];  
...
$pdf->Cell(50, 6, utf8_decode('FECHA: ' . ' ' . $fecha_y), 1,1, 'C');


$pdf->SetFont('Arial', 'B', 10);
$pdf->MultiCell(190, 6, utf8_decode('NOTAS'), 1, 'C');
$pdf->MultiCell(190, 6, utf8_decode($tip_ey), 1, 'C');


$pdf->SetFont('Arial', '', 10);
$pdf->MultiCell(190, 6, utf8_decode($problemay), 1, 'L');
$pdf->MultiCell(190, 6, utf8_decode($subjetivoy), 1, 'L');
$pdf->MultiCell(190, 6, utf8_decode($objetivoy), 1, 'L');
$pdf->MultiCell(190, 6, utf8_decode($analisisy), 1, 'L');

$pdf->MultiCell(190, 6, utf8_decode($plany), 1, 'L');

$pdf->MultiCell(190, 6, utf8_decode($pxy), 1, 'L');
$pdf->Ln(3);
