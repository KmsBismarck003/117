...
$fecha_actual = date("Y-m-d H:i:s");
    ...
    //$Id_exp = ($_POST['Id_exp']);
    //  $fec_hc = ($_POST['fec_hc']);
    $problemay = ($_POST['problemay']);
    $subjetivoy = ($_POST['subjetivoy']);
    $objetivoy = ($_POST['objetivoy']);
    $analisisy = ($_POST['analisisy']);
    $plany = ($_POST['plany']);
    $pxy = ($_POST['pxy']);
    $tip_ey = ($_POST['tip_ey']);
     $dest_cu_yesos = ($_POST['dest_cu_yesos']);
...
$ingresar = mysqli_query($conexion, 'INSERT INTO dat_yesos (fecha_y,id_atencion,tip_ey,problemay,subjetivoy,objetivoy,analisisy,plany,pxy,dest_cu_yesos,id_usua) values (" ' . $fecha_actual . '",' . $id_atencion . ' ," ' . $tip_ey . '" ," ' . $problemay . '" ," ' . $subjetivoy . '" , "' . $objetivoy . '" , "' . $analisisy . '" , "' . $plany . '" , "' . $pxy . ' ","' . $dest_cu_yesos . ' ",' . $id_usua . ') ') or die('<p>Error al registrar</p><br>' . mysqli_error($conexion));

$ingresar=mysqli_query($conexion,'UPDATE dat_ingreso SET area = "'.$dest_cu_yesos.'" WHERE id_atencion = "'.$id_atencion.'" ');


if($dest_cu_yesos="QUIROFANO"){
