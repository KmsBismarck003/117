        ...
        <div class="table-responsive">
          ...
          <table class="table table-bordered table-striped" id="mytable">
            ...
            <tbody>
              ...
              // $sql = "SELECT id_usua, curp_u, nombre, papell,sapell,fecha,mat,cedp,cargp,email,u_activo FROM reg_usuarios;";
              //  $result = $conn->query($sql);
              //$resultado2 = $conexion->query("SELECT P.Id_exp, P.nom_pac, P.papell, P.sapell, DI.id_atencion  FROM paciente P, dat_ingreso DI WHERE  DI.activo ='SI' AND DI.Id_exp=P.Id_exp") or die($conexion->error);

              $resultado2 = $conexion->query("SELECT * FROM dat_ingreso d, paciente p where p.Id_exp=$id_exp and d.Id_exp=$id_exp ") or die($conexion->error);
              while ($row = $resultado2->fetch_assoc()) {
                echo '<tr>'
                  ...
                  . '<td> <strong>
                  <a type="submit" class="btn btn-danger btn-sm"
                     href="pdf_consent_covid.php?id_exp=' . $row['Id_exp']  . '&id_atencion=' . $row['id_atencion'] . '"
                     target="_blank"><span class="fa fa-file-pdf-o"
                                           style="font-size:20px"></span></a>
              </strong></td>'
                  . '<td> <strong>
          ...
          . '<td> <strong>
                  <a type="submit" class="btn btn-danger btn-sm"
                     href="pdf_ficha_deposito.php?id_exp=' . $row['Id_exp'] . '&id_atencion=' . $row['id_atencion'] . '"
                     target="_blank"><span class="fa fa-file-pdf-o"
                                           style="font-size:20px"></span></a>
              </strong></td>'
          . '<td> <strong>
              <a type="submit" class="btn btn-danger btn-sm"
                 href="pdf_completo.php?id_exp=' . $row['Id_exp'] . '&id_atencion=' . $row['id_atencion'] . '"
                 target="_blank"><span class="fa fa-file-pdf-o"
                                       style="font-size:20px"></span></a>
          </strong></td>'
                  . '</tr>';
