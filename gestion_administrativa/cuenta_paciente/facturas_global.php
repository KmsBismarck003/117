...
//$resultado4 = $conexion->query("SELECT *,id_atencion,sum(precio) as suma FROM gen_concepto where id_atencion = id_atencion GROUP BY id_atencion") or die($conexion->error);
              ...
              $resultado4 = $conexion->query("SELECT * FROM comprobantes c,gen_factura g WHERE c.id_atencion=g.id_atencion ORDER by c.id_comp ASC") or die($conexion->error);
...
echo '<td><center><strong><a type="submit" class="btn btn-success btn-sm"
    ...
    $resultadocom = $conexion->query("SELECT * FROM Complemento") or die($conexion->error);
...
$id_complemento=$row4c['id_complemento'];
                ...
                if(isset($id_datfin)){ 
                  echo '<td><strong><a type="submit" class="btn btn-danger btn-sm"
                     href="pdf_factura.php?id_atencion=' . $row4['id_atencion'] . '&id_dat_gen_f=' . $row4['id_comp'].'&id_usua='.$id_usua .'&id_datfin='.$id_datfin .'"
                     target="_blank" title="Ver"><span class="fa fa-file-pdf-o"</span></a></strong></td>';
                   }else{
                    echo '<td><strong><a type="submit" class="btn btn-danger btn-sm"
                     href="pdf_factura.php?id_atencion=' . $row4['id_atencion'].'"
                     target="_blank" title="Ver"><span class="fa fa-file-pdf-o"</span></a></strong></td>';
                   }
