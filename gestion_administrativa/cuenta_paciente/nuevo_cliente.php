...
<strong><center>NUEVO CLIENTE</center></strong>
</div>
<p>
<!--<div class="container">
  <div class="row">
...
<input type="text" name="nom_c" class="form-control" required>
    </div>
...
<input type="text" name="razon_s" class="form-control" required>
    </div>
    <div class="col-sm">
    <strong>Calle</strong>
...
<select name="reg_fiscal" class="form-control" data-live-search="true" id="mibuscador3" style="width : 100%; heigth : 100%">
         ...
         //include "../conexionbdf.php";
                ...
                $result_diag=$conexion->query($sql_diag);
                while($row=$result_diag->fetch_assoc()){
                echo "<option value='" . $row['c_RegimenFiscal'] . "'>".$row['Descripcion']."</option>"; 
                } ?>
            </select>
    </div>
  </div>
...
<hr>
<center>
<input type="submit" class="btn btn-success" role="button" name="guardar" value="Guardar cliente">
</center>
</form>
</div>
...
include "../../conexionbd.php";
     $sql_estado = $conexion->query("SELECT * FROM estados where id_edo=$estado") or die($conexion->error);
              while ($rowe = $sql_estado->fetch_assoc()) { 
                 $nom_estado=$rowe['nombre'];
                }
$sql_estado = $conexion->query("SELECT * FROM municipios where id_mun=$municipio") or die($conexion->error);
              while ($rowe = $sql_estado->fetch_assoc()) { 
                 $nom_mun=$rowe['nombre_m'];
                }
               

  include "conexionbdf.php";
$insertcon=mysqli_query($conexion,'INSERT INTO c_cliente(rfc,razon_s,calle,no_ext,no_int,estado,municipio,cod_postal,asentamiento,reg_fiscal,id_usua) values ("'.$rfc.'","'.$razon_s.'","'.$calle.'","'.$no_ext.'","'.$no_int.'","'.$nom_estado.'","'.$nom_mun.'","'.$cod_postal.'","'.$asenta.'","'.$reg_fiscal.'","'.$id_usua.'")') or die ('<p>Error al registrar</p><br>' . mysqli_error($conexion));
