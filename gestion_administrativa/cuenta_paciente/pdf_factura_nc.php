<?php
//use PDF as GlobalPDF;

require '../../fpdf/fpdf.php';


$id_atencion = @$_GET['id_atencion'];
$id_exp = @$_GET['id_exp'];
$id_datfin=@$_GET['id_datfin'];
$iddat=@$_GET['iddat'];



//$id_usua=@$_GET['id_usua'];
include "conexionbdf.php";

mysqli_set_charset($conexion, "utf8");


class PDF extends FPDF
{
    
  function Header()
  {


   $id_atencion = @$_GET['id_atencion'];
   include "conexionbdf.php";
   $id_f=@$_GET['id_f'];
$iddat=@$_GET['iddat'];
$id_nc=@$_GET['id_nc'];
$sql_dat_ingca = "SELECT * from comprobante_nc c, nota_credito n where c.id_atencion=n.id_atencion and n.id_dat_gen_f=c.id_dat_gen_f and c.id_nc='$id_nc' and n.id_nc='$id_nc'";
$result_dat_ingca = $conexion->query($sql_dat_ingca);
while ($row_dat_ingca= $result_dat_ingca->fetch_assoc()) {
    $cadenaor=$row_dat_ingca['cadena_or'];
    $fechatim=$row_dat_ingca['fecha'];
     
    $uuid=$row_dat_ingca['uuid_nc'];
    $nocertificado=$row_dat_ingca['no_certificado'];
    $sellocfd=$row_dat_ingca['sello_cfd'];
    $sellosat=$row_dat_ingca['sello_sat'];
     
    $reg_fiscal=$row_dat_ingca['reg_fiscal_nc'];
    $rfc_receptor=$row_dat_ingca['rfc_receptor_nc'];
    $razon_s=$row_dat_ingca['razon_s'];
  
    $folio_nc=$row_dat_ingca['folio_nc'];
    $serie_nc=$row_dat_ingca['serie_nc'];
    $uso_cfdi_nc=$row_dat_ingca['uso_cfdi_nc'];
    $tipo_cfdi_nc=$row_dat_ingca['tipo_cfdi_nc'];
    $paciente=$row_dat_ingca['paciente'];
    
    $uuid_relacionado=$row_dat_ingca['uuid_relacionado'];
    $tipo_relacion_nc=$row_dat_ingca['tipo_relacion_nc'];
    

        
}
   
    $this->Image('../../imagenes/SI.PNG', 3, 10, 50, 17);
    $this->SetFont('Arial', 'B', 9);
    $this->Cell(42.5, 8, '', 0, 0, 'C');
    $this->Cell(17.5, 8, 'EMISOR:', 0, 0, 'C');
    $this->SetFont('Arial', '', 9);
    $this->Cell(1, 8, 'CLINICA MEDICA SI SC', 0, 0, 'L');
    
    
    $this->Ln(4);
    $this->SetFont('Arial', 'B', 9);
    $this->Cell(36, 8, '', 0, 0, 'C');
    $this->Cell(24.5, 8, 'RFC:', 0, 0, 'C');
    $this->SetFont('Arial', '', 9);
    $this->Cell(1, 8, 'CMS1501012H9', 0, 0, 'L');
 $this->Cell(85, 8, '', 0, 0, 'L');
      $this->SetFont('Arial', 'B', 11);
      
        $this->Cell(17.5, 8, utf8_decode('NOTA DE CRÉDITO'), 0, 0, 'C');
 $this->Ln(4);
    $this->SetFont('Arial', 'B', 9);
    $this->Cell(44, 8, '', 0, 0, 'C');
    $this->Cell(30, 8, 'REGIMEN FISCAL: ', 0, 0, 'C');
     $this->SetFont('Arial', '', 9);
        $this->Cell(30, 8, $reg_fiscal, 0, 0, 'L');
        
$this->Ln(5.5);
    $this->SetFont('Arial', 'B', 9);
$this->Cell(43.5, 6, utf8_decode(""),0, 'C');
$this->Cell(20, 6, utf8_decode("RECEPTOR:"),0, 'L');
    $this->SetFont('Arial', '', 7.2);
$this->Cell(22, 6, utf8_decode($razon_s),0, 'L');

$this->Ln(5);
$this->SetFont('Arial', 'B', 9);
$this->Cell(43.5, 6, utf8_decode(""),0, 'C');
$this->Cell(10, 6, utf8_decode("RFC:"),0, 'C');
$this->SetFont('Arial', '', 9);
$this->Cell(21, 6, utf8_decode($rfc_receptor),0, 'L');




$this->SetY(4);
 $this->SetFont('Arial', 'B', 13);
 $this->SetFillColor(155, 194, 255);
 $this->Cell(135, 8, utf8_decode(''), 0, 0, 'C');
$this->Cell(60, 6, utf8_decode('Médica San Isidro'), 0, 0, 'C',True);
$this->Ln(6);
$this->SetFont('Arial', 'B', 9);
$this->Cell(140, 8, utf8_decode(""), 0, 0, 'C');
$this->Cell(18, 8, utf8_decode('FACTURA:' . $serie_nc . ' ' . $folio_nc), 0, 0, 'C');
$this->Cell(5, 8, utf8_decode(""), 0, 0, 'C');
$this->Cell(35, 8, utf8_decode('Versión: 4.0'), 0, 0, 'C');

$this->Ln(11);
$this->SetFont('Arial', 'B', 9);
$this->Cell(142.4, 8, utf8_decode(""), 0, 0, 'C');
$this->Cell(13, 6, utf8_decode("LUGAR: "),0, 'C');
$this->SetFont('Arial', '', 9);
$this->Cell(10, 6, utf8_decode("52140"),0, 'C');
$this->Ln(5);

$this->SetFont('Arial', 'B', 9);
$this->Cell(142.4, 8, utf8_decode(""), 0, 0, 'C');
$this->Cell(13, 6, utf8_decode("FECHA: "),0, 'C');
$this->SetFont('Arial', '', 9);
$fec=date_create($fecha);
$this->Cell(10, 6, utf8_decode(date_format($fec,'d/m/Y H:i:s a')),0, 'C');


$this->Ln(9);

if($uso_cfdi_nc=="G03"){
    $usoc="GASTOS EN GENERAL";
}else if($uso_cfdi_nc=="D02"){
    $usoc="GASTOS MÉDICOS POR INCAPACIDAD O DISCAPACIDAD";
}else if($uso_cfdi_nc=="D01"){
    $usoc="HONORARIOS MÉDICOS, DENTALES Y GASTOS HOSPITALARIOS";
}else if($uso_cfdi_nc=="S01") {
     $usoc="SIN EFECTOS FISCALES";
}else if($uso_cfdi_nc=="G02") {
     $usoc="DEVOLUCIONS, DESCUENTOS O BONIFICACIONES";
}

$this->SetFont('Arial', 'B', 9);
$this->Cell(18, 6, utf8_decode("USO CFDI:"),0, 'C');
$this->SetFont('Arial', '', 8);
$this->Cell(124.5, 6, utf8_decode($uso_cfdi_nc . " - " .$usoc),0, 'L');

$this->SetFont('Arial', 'B', 9);
$this->Cell(20, 6, utf8_decode("TIPO CFDI:"),0, 'C');
$this->SetFont('Arial', '', 9);
if ($tipo_cfdi_nc=="I") {
  $tipo_cfdi_nc="I -INGRESO";
}else if ($tipo_cfdi_nc=="E") {
  $tipo_cfdi_nc="E - EGRESO";
}else if ($tipo_cfdi_nc=="T") {
  $tipo_cfdi_nc="T -TRASLADO";
}else if ($tip_cfdi=="N") {
  $tipo_cfdi_nc="N -NOMINA";
}else if ($tipo_cfdi_nc=="P") {
  $tipo_cfdi_nc="P -PAGO";
}

$this->Cell(37, 6, utf8_decode($tipo_cfdi_nc),0, 'C');
$this->SetFont('Arial', 'B', 8);
$this->Ln(5);
$this->Cell(104, 6, utf8_decode(""),0, 'C');
$this->Cell(22, 6, utf8_decode("FOLIO FISCAL: "),0, 'C');
$this->SetFont('Arial', '', 8);
$this->Cell(10, 6, utf8_decode($uuid),0, 'C');


include '../../conexionbd.php';

$this->Ln(6);

$this->SetFont('Arial', 'B', 8);
$this->Cell(17, 6, utf8_decode("PACIENTE:"),0, 'C');
$this->SetFont('Arial', '', 8);
$this->Cell(30, 6, utf8_decode($paciente),0, 'L');

$this->SetFont('Arial', 'B', 8);
$this->Cell(57.3, 6, utf8_decode(""),0, 'C');
$this->Cell(27, 6, utf8_decode("NO. CERTIFICADO: "),0, 'C');
$this->SetFont('Arial', '', 8);
$this->Cell(10, 6, utf8_decode($nocertificado),0, 'L');
$this->Ln(6);

$this->SetFont('Arial', 'B', 8);
$this->Cell(31, 6, utf8_decode("CFDI RELACIONADO:"),0, 'C');
$this->SetFont('Arial', '', 8);
$this->Cell(16, 6, utf8_decode($uuid_relacionado),0, 'L');

$this->SetFont('Arial', 'B', 7);
$this->Cell(57.5, 6, utf8_decode(""),0, 'C');
$this->Cell(26, 6, utf8_decode("TIPO DE RELACIÓN: "),0, 'C');
$this->SetFont('Arial', '', 7);
if($tipo_relacion_nc=="01"){
    $tipo_r_nc="Nota de crédito de los documentos relacionados";
}else if($tipo_relacion_nc=="03"){
     $tipo_r_nc="Devolución de mercancía sobre facturas o traslados previos";
}

$this->Cell(10, 6, utf8_decode($tipo_relacion_nc."-".$tipo_r_nc),0, 'L');
include "conexionbdf.php";




   $this->SetDrawColor(0, 0, 0);
    $this->Line(69, 7, 140, 7);
    $this->Line(69, 8, 140, 8);
  }
  function Footer()
  {
    $this->Ln(8);
    $this->SetY(-15);
     $this->Cell(0, 10, utf8_decode('Este documento es una representación impresa de un CFDI'), 0, 0, 'C');
    $this->Cell(0, 10, utf8_decode('Página ' . $this->PageNo() . '/{nb}'), 0, 0, 'C');
  }
}
include "conexionbdf.php";

 $id_f=@$_GET['id_f'];







$pdf = new PDF('P');
$pdf->AliasNbPages();
$pdf->AddPage();

include "conexionbdf.php";

$pdf->Ln(6);

$pdf->SetFont('Arial', 'B', 8);
$pdf->SetFillColor(155, 194, 255); // establece el color del fondo de la celda (en este caso es AZUL
$pdf->Cell(15, 4, utf8_decode("Código"),0,0, 'L','True');
$pdf->Cell(104, 4, utf8_decode("Concepto"),0,0, 'L','True');
$pdf->Cell(11, 4, utf8_decode("Clave"),0,0, 'L','True');
$pdf->Cell(10, 4, utf8_decode("U.M."),0,0, 'L','True');
$pdf->Cell(15, 4, utf8_decode("Cantidad"),0,0, 'L','True');
$pdf->Cell(20, 4, utf8_decode("P.U."),0,0, 'L','True');
$pdf->Cell(20, 4, utf8_decode("Importe"),0,0, 'L','True');

      
include "conexionbdf.php";

$iddat=@$_GET['iddat'];

$id_nc=@$_GET['id_nc'];


$sql_dat_ingca = "SELECT * from comprobante_nc c, nota_credito n where c.id_atencion=n.id_atencion and n.id_dat_gen_f=c.id_dat_gen_f and c.id_nc='$id_nc' and n.id_nc='$id_nc'";
$result_dat_ingca = $conexion->query($sql_dat_ingca);
while ($row_dat_ingca= $result_dat_ingca->fetch_assoc()) {
    $cadenaor=$row_dat_ingca['cadena_or'];
    $fechatim=$row_dat_ingca['fecha'];
     
    $uuid=$row_dat_ingca['uuid_nc'];
    $nocertificado=$row_dat_ingca['no_certificado'];
    $sellocfd=$row_dat_ingca['sello_cfd'];
    $sellosat=$row_dat_ingca['sello_sat'];
     
    $reg_fiscal=$row_dat_ingca['reg_fiscal_nc'];
    $rfc_receptor=$row_dat_ingca['rfc_receptor_nc'];
    $razon_s=$row_dat_ingca['razon_s'];
  
    $folio_nc=$row_dat_ingca['folio_nc'];
    $serie_nc=$row_dat_ingca['serie_nc'];
    $uso_cfdi_nc=$row_dat_ingca['uso_cfdi_nc'];
    $tipo_cfdi_nc=$row_dat_ingca['tipo_cfdi_nc'];
    $paciente=$row_dat_ingca['paciente'];
    
    //CONCEPTO
    $concepto_nc=$row_dat_ingca['concepto_nc'];
    $descuento=$row_dat_ingca['descuento'];
    $concepto_nc=$row_dat_ingca['concepto_nc'];
    
    $met_pago=$row_dat_ingca['met_pago'];
    $forma_pago_nc=$row_dat_ingca['forma_pago_nc'];
}



  $pdf->Ln(3);
  $pdf->SetFont('Arial', '', 7);
$pdf->Cell(15, 6, utf8_decode('84111506'),0, 'C');
$pdf->Cell(104, 6, utf8_decode($concepto_nc),0, 'C');
$pdf->Cell(11, 6, utf8_decode('ACT'),0, 'C');
$pdf->Cell(10, 6, utf8_decode('SER'),0, 'C');
$pdf->Cell(15, 6, utf8_decode(1.00),0, 'C');
$pdf->Cell(20, 6, utf8_decode("$".$descuento),0, 'L');
$pdf->Cell(20, 6, utf8_decode("$".$descuento),0, 'L');

 $pdf->Ln(3);





include "conexionbdf.php";
$pdf->Ln(6);
$pdf->SetFont('Arial', '', 10);
$pdf->Line(5, 283, 205, 283);//DE HASTA ABAJO LINEA

 $pdf->SetFillColor(107, 109, 111);
 $pdf->Cell(195, 0.5, utf8_decode(''), 0, 0, 'C','True');

$pdf->Ln(4);
$pdf->Cell(-0.5,69, $pdf->Image('../cuenta_paciente/codigoQr.jpg', $pdf->GetX(), $pdf->GetY(),30),0);
$pdf->SetFont('Arial', 'B', 8);
$pdf->Cell(130, 6, utf8_decode(""),0, 'C');


$pdf->Cell(49, 6, utf8_decode("SUBTOTAL CUENTA:"),0, 'R');
$pdf->SetFont('Arial', '', 8);
$pdf->Cell(20, 6, utf8_decode("$".$descuento),0, 'L');


$pdf->SetFont('Arial', 'B', 8);
//$pdf->Cell(130, 6, utf8_decode(""),0, 'C');
//$pdf->Cell(39, 6, utf8_decode("IVA:"),0, 'R');
//$pdf->Cell(10, 6, utf8_decode(""),0, 'C');

$pdf->SetFont('Arial', '', 8);
  //$pdf->Cell(20, 6, utf8_decode("$".$english_format_number = number_format($iva, 2)),0, 'R');

$pdf->Ln(6);
 $pdf->Cell(130, 0.5, utf8_decode(''), 0, 0, 'C');
$pdf->SetFillColor(107, 109, 111);
 $pdf->Cell(65, 0.5, utf8_decode(''), 0, 0, 'C','True');
 $pdf->Ln(1);
$pdf->SetFont('Arial', 'B', 8);
$pdf->Cell(130, 6, utf8_decode(""),0, 'C');
$pdf->Cell(49, 6, utf8_decode("TOTAL:"),0, 'R');
$pdf->SetFont('Arial', '', 8);
$pdf->Cell(29, 6, utf8_decode("$".$english_format_number = number_format($descuento, 2)),0, 'R');


$pdf->Ln(25);

if($met_pago=="PUE"){
    $metp="PAGO EN UNA SOLA EXHIBICIÓN";
}else if($met_pago=="PPD"){
    $metp="PAGO EN PARCIALIDADES O DIFERIDO";
}


$pdf->SetFont('Arial', 'B', 7);
$pdf->Cell(29, 6, utf8_decode("MÉTODO DE PAGO:"),0, 'C');
$pdf->SetFont('Arial', '', 7);
$pdf->Cell(29, 6, utf8_decode($met_pago . " - " . $metp),0, 'C');

$pdf->Ln(4);

if($forma_pago_nc==01){
    $fp="EFECTIVO";
}else if($forma_pago_nc==99){
    $fp="POR DEFINIR";
}else if($forma_pago_nc==04){
    $fp="TARJETA DE CRÉDITO";
}else if($forma_pago_nc==28){
    $fp="TARJETA DE DÉBITO";
}else if($forma_pago_nc==03){
    $fp="TRANSFERENCIA ELECTRÓNICA DE FONDOS";
}

$pdf->SetFont('Arial', 'B', 7);
$pdf->Cell(29, 6, utf8_decode("FORMA DE PAGO:"),0, 'C');
$pdf->SetFont('Arial', '', 7);
$pdf->Cell(29, 6, utf8_decode($forma_pago_nc . ' - ' . $fp),0, 'C');


$pdf->Ln(6);
 $pdf->SetFillColor(107, 109, 111);
 $pdf->Cell(195, 0.5, utf8_decode(''), 0, 0, 'C','True');
$pdf->Ln(1);

$pdf->SetFont('Arial', 'B', 7);
$pdf->Cell(85, 6, utf8_decode("SELLO DIGITAL CFD:"),0, 'C');
$pdf->Ln(5);
$pdf->SetFont('Arial', '', 6);
$pdf->MultiCell(194, 3, utf8_decode($sellocfd),0, 'J');


$pdf->SetFont('Arial', 'B', 7);
$pdf->Cell(85, 6, utf8_decode("SELLO DIGITAL DEL SAT:"),0, 'C');
$pdf->Ln(5);
$pdf->SetFont('Arial', '', 6);
$pdf->MultiCell(194, 3, utf8_decode($sellosat),0, 'J');


$pdf->SetFont('Arial', 'B', 7);
$pdf->Cell(85, 6, utf8_decode("CADENA ORIGINAL DEL COMPLEMENTO DE CERTIFICACIÓN DEL SAT:"),0, 'C');
$pdf->Ln(5);
$pdf->SetFont('Arial', '', 6);
$pdf->MultiCell(194, 3, utf8_decode($cadenaor),0, 'J');
$pdf->Ln(1);
$pdf->SetFillColor(107, 109, 111);
 $pdf->Cell(195, 0.5, utf8_decode(''), 0, 0, 'C','True');

$ftim=date_create($fechatim);
$fs=date_format($ftim,"Y-m-d");

$htim=date_create($fechatim);
$hs=date_format($htim,"H:i:s");


$fs;
list($anio, $mes, $dia) = explode("-",$fs);
$Fs=$anio.'-'.$mes.'-'.$dia;
$FechaCompleta=$Fs.'T'.$hs;

$pdf->Ln(1);
$pdf->SetFont('Arial', '', 7);
$pdf->Cell(85, 6, utf8_decode("FECHA Y HORA DE TIMBRADO: " .$FechaCompleta),0, 'C');



$pdf->Output();
