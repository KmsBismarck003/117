...
$insertconc=mysqli_query($conexion,'INSERT INTO gen_concepto_fact(id_atencion,id_usua,codigo,cantidad,descripcion,codescded,iva,precio,importe_traslado,clave_unidad,unidad,obj_impuesto,fecha,tip_concepto,total) values ('.$id_at.','.$id_usua.',"85101501","1","SERVICIOS HOSPITALARIOS DE EMERGENCIA O QUIRÚRGICOS","'.$codescdedl.'","'.$iva.'","'.$tprec.'","'.$trasl.'","E48","SERVICIO","'.$obj_impuestoo.'","'.$fecha_actual.'","Global","'.$tot.'")') or die ('<p>Error al registrar</p><br>' . mysqli_error($conexion));
...
$insertconc=mysqli_query($conexion,'INSERT INTO gen_concepto_fact(id_atencion,id_usua,codigo,cantidad,descripcion,codescded,iva,precio,importe_traslado,clave_unidad,unidad,obj_impuesto,fecha,tip_concepto,total) values ('.$id_at.','.$id_usua.',"85101501","1","SERVICIOS HOSPITALARIOS DE EMERGENCIA O QUIRÚRGICOS","'.$codescdedl.'","'.$IVAd.'","'.$deposito.'","'.$trasl.'","E48","SERVICIO","'.$obj_impuestoo.'","'.$fecha_actual.'","Global","'.$totfina.'")') or die ('<p>Error al registrar</p><br>' . mysqli_error($conexion));
...
$obj_impuesto= mysqli_real_escape_string($conexion, (strip_tags($_POST["obj_impuesto"], ENT_QUOTES)));
        $xml .= '<cfdi:Concepto ClaveProdServ="85101501" Cantidad="1" ClaveUnidad="E48" Unidad="SERVICIO" Descripcion="SERVICIOS HOSPITALARIOS DE EMERGENCIA O QUIRÚRGICOS" ValorUnitario="'.$english_format_number = number_format($SUB, 6, '.', '').'" Importe="'.$english_format_number = number_format($SUB, 6, '.', '').'" Descuento="'.$english_format_number = number_format($cdesc, 6, '.', '').'" ObjetoImp="'.$obj_impuestoo.'">
...
$insertconc=mysqli_query($conexion,'INSERT INTO Complemento(id_atencion,id_usua,fecha,bancoor,cuentaor,montoant,saldoant,noparcialidad,comentario,serie,folio,moneda,formapago,objetoimp,clave_unidad,unidad,codigo) values ('.$id_at.','.$id_usua.',"'.$fecha_actual.'","'.$bancoor.'","'.$cuentaor.'","'.$montoant.'","'.$saldoant.'","'.$noparcialidad.'","'.$comentario.'","E","'.$folio.'","'.$moneda.'","'.$forma_pago.'","'.$obj_impuesto.'","'. $row['clave_unidad'].'","'.$row['unidad'].'","'.$row['codigo'].'")') or die ('<p>Error al registrar</p><br>' . mysqli_error($conexion));
...
echo '<script type="text/javascript">window.location.href ="facturacion_pnormal.php?id_atencion='.$id_at.'" ;</script>';
