...
$saldoant = $_GET['saldoant'];
$id_dat_gen_f = $_GET['id_dat_gen_f'];
...
<div class="container">
  <div class="row">
    <div class="col-sm">
    <strong>*Serie</strong>
<input type="text" name="serie" value="D" class="form-control" disabled>
    </div>
    <!--<div class="col-sm">
...
<select name="uso_cfdi" class="form-control" data-live-search="true" id="mibuscador3" style="width : 100%; heigth : 100%">
              <option value="">Seleccionar</option>
                <?php
         //include "../conexionbdf.php";
                $sql_diag="SELECT * FROM c_usocfdi where Descripcion='Gastos en general' or Descripcion='Honorarios médicos, dentales y gastos hospitalarios' or Descripcion='Gastos médicos por incapacidad o discapacidad' or Descripcion='Pagos' order by Descripcion ASC";
                $result_diag=$conexion->query($sql_diag);
                while($row=$result_diag->fetch_assoc()){
...
<select name="tip_cfdi" class="form-control" data-live-search="true" style="width : 100%;heigth:100%;">
                ...
                <?php
         include "../conexionbdf.php";
                $sql_diag="SELECT * FROM c_tipodecomprobante where Descripcion='Pago'";
                $result_diag=$conexion->query($sql_diag);
                while($row=$result_diag->fetch_assoc()){
...
<div class="container">
  <div class="row">
        <div class="col-sm-6">
<strong>Pago complemento</strong>
...
echo '<strong>Subtotal con IVA: ' . $english_format_number = number_format($row['importe'], 2, '.', '').'</strong>';
              }   
...
echo '<strong>Subtotal sin IVA: ' . $row['importe_noiva'].'</strong>';
...
<select name="obj_impuestoo" class="form-control" data-live-search="true" id="mibuscador3" style="width : 100%; heigth : 100%">
              <option value="">Seleccionar</option>
                <?php
         //include "../conexionbdf.php";
                $sql_diag="SELECT * FROM c_objetoimp WHERE c_ObjetoImp=01";
                $result_diag=$conexion->query($sql_diag);
                while($row=$result_diag->fetch_assoc()){
...
<tr> 
<td class="fondo"><strong><font size="4">PAQUETE DE SERVICIO HOSPITALARIO<?php //echo $row['descripcion'];?></font></strong>
...
<br><strong>Subtotal:</strong> $<?php echo $english_format_number = number_format($deposito, 2, '.', '')?></td>
...
echo "25*0.1: " . $english_format_number = number_format($dasegg=floor(($rowim["base"])*($daseg)*100)/100, 2, '.', ''); //subtototal * aseguradora
 echo  '<br>';
echo "traslado: " . $english_format_number = number_format($sumadescb=floor((($rowim["base"])-($dasegg)-100)*100)/100, 2, '.', ''); //suma descuentos arriba esto va en descuento 2
echo  '<br>';
...
// insert data
$insertconc=mysqli_query($conexion,'INSERT INTO comprobantes(id_atencion,id_usua,fecha,cadenaor,sellosat,sellocfd,nocertificado,version, subtotal, total, moneda, sello, rfc_emisor, rfc_receptor,uuid,iva,descuento,descuento_aseg) values ('.$id_at.','.$id_usua.',"'.$xml_data["fecha"].'","'.$cadenaOriginal.'","'.$xml_data["sellosat"] .'","'.$xml_data["cfd"].'","'.$xml_data["nocertificado"].'","'.$xml_data["version"].'","'.$xml_data["subtotal"].'","'.$xml_data["total"].'","'.$xml_data["moneda"].'","'.$xml_data["sello"].'","'.$xml_data["rfc_emisor"].'","'.$xml_data["rfc_receptor"].'","'.$xml_data["uuid"].'","'.$xml_data["impuestos"].'","'.$codescded.'","'.$dasegg.'")') or die ('<p>Error al registrar</p><br>' . mysqli_error($conexion));
...
echo '<script type="text/javascript">window.location.href ="facturacioncom.php?id_atencion='.$id_at.'" ;</script>';
...
$insertconc=mysqli_query($conexion,'INSERT INTO gen_concepto_fact(id_atencion,id_usua,codigo,cantidad,descripcion,codescded,iva,precio,importe_traslado,clave_unidad,unidad,obj_impuesto,fecha,tip_concepto,total) values ('.$id_at.','.$id_usua.',"'.$row['codigo'].'","1","PAQUETE DE SERVICIO HOSPITALARIO","'.$codescdedl.'","'.$iva.'","'.$tprec.'","'.$trasl.'","'. $row['clave_unidad'].'","'.$row['unidad'].'","'.$obj_impuesto.'","'.$fecha_actual.'","Global","'.$tot.'")') or die ('<p>Error al registrar</p><br>' . mysqli_error($conexion));



$resultadof1 = $conexion->query("SELECT * from Complemento where id_atencion=$id_at") or die($conexion->error);

                while($f3f = mysqli_fetch_array($resultadof)){
$id_complemento=$f3f1['id_complemento'];
...
$xml ='<?xml version="1.0" encoding="utf-8"?>
<cfdi:Comprobante xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" Moneda="'.$moneda.'" TipoCambio="1" Total="'.$english_format_number = number_format($tot, 2, '.', '').'" xsi:schemaLocation="http://www.sat.gob.mx/cfd/4 http://www.sat.gob.mx/sitio_internet/cfd/4/cfdv40.xsd" Exportacion="01" MetodoPago="'.$metodo_pago.'" Descuento="'.$english_format_number = number_format($sumadesc, 2, '.', '').'" TipoDeComprobante="'.$tip_cfdi.'" SubTotal="'.$english_format_number = number_format($deposito, 2, '.', '').'" FormaPago="'.$forma_pago.'" LugarExpedicion="52140" Fecha="'.$FechaCompleta.'" Folio="'.$folio.'" Serie="E" Version="4.0" xmlns:cfdi="http://www.sat.gob.mx/cfd/4">
...
$importeconceptos=($iim*.16);
 $cdesc=($sumadesc)*($iim);
        ...
        $xml .= '<cfdi:Concepto ClaveProdServ="'.$codigp.'" Cantidad="1" ClaveUnidad="'.$clave_unidad.'" Unidad="'.$unidad.'" Descripcion="PAQUETE DE SERVICIO HOSPITALARIO" ValorUnitario="'.$english_format_number = number_format($iim, 6, '.', '').'" Importe="'.$english_format_number = number_format($iim, 6, '.', '').'" Descuento="'.$english_format_number = number_format($sumadesc, 6, '.', '').'" ObjetoImp="'.$obj_impuesto.'">
...
$insertconc=mysqli_query($conexion,'INSERT INTO gen_concepto_fact(id_atencion,id_usua,codigo,cantidad,descripcion,codescded,iva,precio,importe_traslado,clave_unidad,unidad,obj_impuesto,fecha,tip_concepto,total) values ('.$id_at.','.$id_usua.',"'.$row['codigo'].'","1","PAQUETE DE SERVICIO HOSPITALARIO","'.$codescdedl.'","'.$iva.'","'.$tprec.'","'.$trasl.'","'. $row['clave_unidad'].'","'.$row['unidad'].'","'.$obj_impuesto.'","'.$fecha_actual.'","Global","'.$totar.'")') or die ('<p>Error al registrar</p><br>' . mysqli_error($conexion));
...
$importeconceptos=($iim*.16);
 $cdesc=($sumadesc)*($iim);
        ...
        $xml .= '<cfdi:Concepto ClaveProdServ="'.$row['codigo'].'" Cantidad="1" ClaveUnidad="'.$row['clave_unidad'].'" Unidad="'.$row['unidad'].'" Descripcion="PAQUETE DE SERVICIO HOSPITALARIO" ValorUnitario="'.$english_format_number = number_format($iim, 6, '.', '').'" Importe="'.$english_format_number = number_format($iim, 6, '.', '').'" Descuento="'.$english_format_number = number_format($cdesc, 6, '.', '').'" ObjetoImp="'.$obj_impuesto.'">
...
<div class="collapse" id="div3" style="display:;">
<div class="card card-body">
  <table class="table table-bordered" id="mytable">
                <thead class="thead bg-light">
...
//$resultado4 = $conexion->query("SELECT * FROM dat_financieros where id_atencion = $id_at and id_datfin=$id_datfin") or die($conexion->error);
              //while ($row4 = $resultado4->fetch_assoc()) { ?>
<tr> 
<td class="fondo"><strong><font size="4">PAGO DE SERVICIO HOSPITALARIO<?php //echo $row['descripcion'];?></font></strong>
<!--<br><strong>Clave unidad:</strong> <?php echo $row['clave_unidad'];?></td>-->
...
<br><strong>Subtotal:</strong> $<?php echo $english_format_number = number_format($saldoant, 2, '.', '')?></td>
...
<p>
  ...
  <div class="row">
<div class="col-sm">
     <strong>Banco ordenante</strong>
<input type="text" name="bancoor" class="form-control">
    </div>
   <div class="col-sm">
     <strong>Cuenta ordenante</strong>
<input type="number" step="0.01" name="cuentaor" class="form-control">
    </div>
    <div class="col-sm">
     <strong>*Objeto de Impuesto</strong>
<select name="obj_impuesto" class="form-control" data-live-search="true" id="mibuscador3" style="width : 100%; heigth : 100%">
...
<p>
 <div class="container">
  <div class="row">
      <div class="col-sm">
     <strong>Monto*</strong>
<input type="number" step="0.01" name="montoant" class="form-control">
    </div>
      <div class="col-sm">
     <strong>Importe saldo anterior</strong>
<input type="number" step="0.01" name="saldoant" class="form-control" value="<?php echo $english_format_number = number_format($saldoant, 2, '.', '')?>">
    </div>
<div class="col-sm">
     <strong>Número de parcialidad</strong>
<input type="number" step="0.01" name="noparcialidad" class="form-control">
    </div>
    </div>
    </div>
    <p></p>
  ...
  <div class="row">
      <div class="col-sm">
          <strong>Comentarios de la remisión</strong>
...
<div class="row">
    ...
    <div class="col-sm">
<input type="submit" name="factpago" class="btn btn-primary" value="Guardar factura">
...
//INDISTRIA ILUMINADORA DE ALMACENES   CLINICA MEDICA SI CMS1501012H9

//IIA040805DZ4 

$insertconc=mysqli_query($conexion,'INSERT INTO gen_concepto_fact(id_atencion,id_usua,codigo,cantidad,descripcion,codescded,iva,precio,importe_traslado,clave_unidad,unidad,obj_impuesto,fecha,tip_concepto,total) values ('.$id_at.','.$id_usua.',"84111506","1","Pago","'.$codescdedl.'","'.$iva.'","'.$tprec.'","'.$trasl.'","ACT","'.$row['unidad'].'","'.$obj_impuesto.'","'.$fecha_actual.'","Complemento","'.$montoant.'")') or die ('<p>Error al registrar</p><br>' . mysqli_error($conexion));

$xml ='<?xml version="1.0" encoding="utf-8"?>

<cfdi:Comprobante xmlns:cfdi="http://www.sat.gob.mx/cfd/4" Version="4.0" Serie="E" Folio="'.$folio.'" Fecha="'.$FechaCompleta.'" LugarExpedicion="52140" SubTotal="0" TipoDeComprobante="'.$tip_cfdi.'" Exportacion="01" xsi:schemaLocation="http://www.sat.gob.mx/cfd/4 http://www.sat.gob.mx/sitio_internet/cfd/4/cfdv40.xsd http://www.sat.gob.mx/Pagos20 http://www.sat.gob.mx/sitio_internet/cfd/Pagos/Pagos20.xsd" Total="0" Moneda="XXX" xmlns:pago20="http://www.sat.gob.mx/Pagos20" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
...
$result_uuid = $conexion->query("SELECT * from comprobantes  WHERE id_atencion=$id_at ORDER BY id_comp DESC") or die($conexion->error);
 while ($rowcom = $result_uuid->fetch_assoc()) {
$uuidc=$rowcom['uuid'];
}

$xml .= '<cfdi:Concepto ClaveProdServ="84111506" Cantidad="1" ClaveUnidad="ACT" Descripcion="Pago" ValorUnitario="0" ObjetoImp="'.$obj_impuesto.'" Importe="0">
</cfdi:Concepto>';
$xml .= '</cfdi:Conceptos>
...
<pago20:Pago Monto="'.$english_format_number = number_format($montoant, 2, '.', '').'" TipoCambioP="1" MonedaP="'.$moneda.'" FormaDePagoP="01" FechaPago="'.$FechaCompleta.'">
<pago20:DoctoRelacionado ObjetoImpDR="'.$obj_impuesto.'" ImpSaldoInsoluto="'.$english_format_number = number_format($saldoant-$montoant, 2, '.', '').'" ImpPagado="'.$english_format_number = number_format($montoant, 2, '.', '').'" ImpSaldoAnt="'.$english_format_number = number_format($saldoant, 2, '.', '').'" NumParcialidad="'.$noparcialidad.'" EquivalenciaDR="1" MonedaDR="'.$moneda.'" IdDocumento="'.$uuidc.'"/>
</pago20:Pago>
</pago20:Pagos>
</cfdi:Complemento>
</cfdi:Comprobante>';
...
$totalc=$saldoant-$montoant;

//UPDATE A COMPROBANTE EL TOTAL DE COMPLEMENTO
//$sql_c = "UPDATE comprobantes SET saldoant = '$totalc' WHERE id_dat_gen_f = $id_dat_gen_f";
//$result_comp = $conexion->query($sql_c);


//nsert a tabla complemento
$insertconc=mysqli_query($conexion,'INSERT INTO Complemento(id_atencion,id_usua,fecha,bancoor,cuentaor,montoant,saldoant,noparcialidad,comentario,serie,folio,moneda,formapago,objetoimp,id_documento,clave_unidad,unidad,codigo,total) values ('.$id_at.','.$id_usua.',"'.$fecha_actual.'","'.$bancoor.'","'.$cuentaor.'","'.$montoant.'","'.$saldoant.'","'.$noparcialidad.'","'.$comentario.'","D","'.$folio.'","'.$moneda.'","'.$forma_pago.'","'.$obj_impuesto.'","'.$uuidc.'","ACT","'.$row['unidad'].'","84111506","'.$totalc.'")') or die ('<p>Error al registrar</p><br>' . mysqli_error($conexion));

$resultadof1 = $conexion->query("SELECT * from Complemento where id_atencion=$id_at") or die($conexion->error);

                while($f3f1= mysqli_fetch_array($resultadof1)){
$id_complemento=$f3f1['id_complemento'];

                }
               $resultadof2 = $conexion->query("SELECT * from gen_factura where id_atencion=$id_at") or die($conexion->error);

                while($f3f2 = mysqli_fetch_array($resultadof2)){
$id_genf=$f3f2['id_dat_gen_f'];
...
$params['usuarioIntegrador'] = 'DHSdSjriIjdZV96Kc1Jztg=='; // interador de msi de ellos
...
$insertconc=mysqli_query($conexion,'INSERT INTO comprobantes(id_atencion,id_usua,fecha,cadenaor,sellosat,sellocfd,nocertificado,version, subtotal, total, moneda, sello, rfc_emisor, rfc_receptor,uuid,tipof,id_dat_gen_f,id_complemento) values ('.$id_at.','.$id_usua.',"'.$xml_data["fecha"].'","'.$cadenaOriginal.'","'.$xml_data["sellosat"] .'","'.$xml_data["cfd"].'","'.$xml_data["nocertificado"].'","'.$xml_data["version"].'","'.$montoant.'","'.$montoant.'","'.$xml_data["moneda"].'","'.$xml_data["sello"].'","'.$xml_data["rfc_emisor"].'","'.$xml_data["rfc_receptor"].'","'.$xml_data["uuid"].'","Complemento","'.$id_genf.'","'.$id_complemento.'")') or die ('<p>Error al registrar</p><br>' . mysqli_error($conexion));
