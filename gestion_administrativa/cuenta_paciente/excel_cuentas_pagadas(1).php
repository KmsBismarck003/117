...
<head>
 ...
 <table class="table table-bordered table-striped" id="mytable">
    ...
    $date = date("d/m/Y");
                         ...
                         $resultado = $conexion->query("SELECT di.fecha as ingreso, di.*, p.* from dat_ingreso di, paciente p  WHERE (fec_egreso BETWEEN '$date1' and '$date2')
                         AND p.Id_exp=di.Id_exp and valida='SI' ORDER BY fec_egreso DESC") or die($conexion->error);
                ...
                if ($centro_costo == 'OBSERVACION' || $centro_costo == 'OBSERVACIÓN')
                    {$total_urg = $total_urg + $subtottal + $iva + $subtotalg12h + $subtotalg12;}
                elseif ($centro_costo == 'HOSPITALIZACION' || $centro_costo == 'HOSPITALIZACIÓN')
                    {$total_hos = $total_hos + $subtottal + $iva + $subtotalg12h + $subtotalg12;}
                elseif ($centro_costo == 'QUIROFANO' || $centro_costo == 'QUIRÓFANO')
                    {$total_qui = $total_qui + $subtottal + $iva + $subtotalg12h + $subtotalg12;}
                elseif ($centro_costo == 'TERAPIA INTENSIVA')
                    {$total_ter = $total_ter + $subtottal + $iva + $subtotalg12h + $subtotalg12;}
                elseif ($centro_costo == 'CONSULTA')
                    {$total_con = $total_con + $subtottal + $iva + $subtotalg12h + $subtotalg12;}
                else
                    {$total_hos = $total_hos + $subtottal + $iva + $subtotalg12h + $subtotalg12;}
            }
