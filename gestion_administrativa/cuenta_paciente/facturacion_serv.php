...
<?php 
       include "../../conexionbd.php";
$sql_pac = "SELECT * from pserv WHERE id_pac=$id_at";

        $result_pac = $conexion->query($sql_pac);

        while ($row_pac = $result_pac->fetch_assoc()) {
          $nombrepacs = $row_pac['nombre'];
          $id_pac = $row_pac['id_pac'];
      
        }
        include "conexionbdf.php";
        
?>
<label class="control-label">Paciente: </label><strong> &nbsp; <?php echo $id_pac.' - '.$nombrepacs?></strong>
...
<nav>
  <div class="nav nav-tabs" id="nav-tab" role="tablist">
      ...
      $resultado341 = $conexion->query("SELECT * from comprobantes where id_atencion=$id_at and externo='Si'") or die($conexion->error);
                while($f341 = mysqli_fetch_array($resultado341)){
      $valid=$f341['id_atencion'];
      $facestatus=$f341['estatus'];
                }
      ?>
      <?php 
      if($facestatus=="activa"){
       
      ?>
 <a class="nav-item nav-link">Cuenta facturada</a>
<?php }else{
?>
...
$id_usua= $usuario['id_usua'];
$id_at = $_GET['id_atencion'];
$resultado = $conexion->query("SELECT * from gen_concepto WHERE id_atencion=$id_at and externo='Si' ORDER BY id_conce DESC") or die($conexion->error);
...
<td class="fondo"><strong> $<?php echo $row['precio']*.16;?></strong></td>
<?php }else{ 
  ?>
    <td class="fondo"><strong> $<?php echo $row['precio']*.16;?></strong></td>
    <td class="fondo"><strong> </strong></td>
...
?>
 ...
 <a href="edita_conc.php?id_co=<?php echo $row['id_conce'];?>&id_atencion=<?php echo $row['id_atencion'];?>"><button type="button" data-toggle="modal" data-target="#exampleModal" class="btn btn-warning"><i class="fa fa-edit" style="font-size:17px" aria-hidden="true"></i> </button></a>
    ...
    $resultado = $conexion->query("SELECT *,sum(importe) as importe, sum(precio) as totinicial from gen_concepto WHERE id_atencion=$id_at  and externo='Si'  group by id_atencion ") or die($conexion->error);
              while ($row = $resultado->fetch_assoc()) {
                  
                  $ivva = $row['precio']*.16;

$totinicial=$row['totinicial'];
$subb=$row['importe'];
...
echo '<strong>Subtotal genera IVA: ' . number_format($row['importe']+$sumaiva, 2).'</strong>';

              
          $resultado = $conexion->query("SELECT *,sum(importe_noiva) as importe_noiva from gen_concepto WHERE id_atencion=$id_at and externo='Si' group by id_atencion ") or die($conexion->error);
              while ($row = $resultado->fetch_assoc()) {
...
//    descuento validacion si existe descuento en dat finacieros restarle a la factura en base e descuento autmatico
 include "../../conexionbd.php";
$resultadoas = $conexion->query("SELECT * from depositos_pserv WHERE id_pac=$id_at and tipo_pago='DESCUENTO'") or die($conexion->error);
              while ($rowas = $resultadoas->fetch_assoc()) {
                $banco=$rowas['tipo_pago'];
                $deposito=$rowas['deposito'];
              }
              
   
             $CanInicial=$subb; // ya esta bien
...
$TOTALPO=$bmiva+$tcondes;
   ?>
   <br><strong>Total inicial:</strong> $<?php echo number_format($totinicial, 2);?><br>
...
$id_usua= $usuario['id_usua'];
$id_at = $_GET['id_atencion'];
$resultado = $conexion->query("SELECT *,sum(importe) as precio from gen_concepto WHERE id_atencion=$id_at  and externo='Si' group by id_atencion ORDER BY id_conce DESC") or die($conexion->error);
              while ($row = $resultado->fetch_assoc()) { 
//include "../../conexionbd.php";
...
<tr> 
<td class="fondo"><strong><font size="4">SERVICIOS HOSPITALARIOS DE EMERGENCIA O QUIRÚRGICOS<?php //echo $row['descripcion'];?></font></strong>
...
$TOTALPO=$bmiva+$tcondes;
    ...
    ?>
    <br><strong>Total cuenta inicial:</strong> $<?php echo number_format($TOTAL=$subb, 2);?>
     <font color="red"><br><strong>Descuento:</strong> $<?php echo number_format($DesSiniva, 2);?></font>
     
  <br><strong>Subtotal:</strong> $<?php echo number_format($CanInicial, 2);?><br>
  
  
    <strong>Importe: </strong>$<?php echo number_format($subglo=$TOTAL/1.16, 2) ?> 
     <br>
...
$resultadoimporte = $conexion->query("SELECT *,sum(importe) as base from gen_concepto WHERE id_atencion=$id_at  and externo='Si' group by id_atencion") or die($conexion->error);
...
$insertconc=mysqli_query($conexion,'INSERT INTO comprobantes(id_atencion,id_usua,fecha,cadenaor,sellosat,sellocfd,nocertificado,version, subtotal, total, moneda, sello, rfc_emisor, rfc_receptor,uuid,iva,descuento,descuento_aseg,montopag,saldoant,id_dat_gen_f) values ('.$id_at.','.$id_usua.',"'.$xml_data["fecha"].'","'.$cadenaOriginal.'","'.$xml_data["sellosat"] .'","'.$xml_data["cfd"].'","'.$xml_data["nocertificado"].'","'.$xml_data["version"].'","'.$xml_data["subtotal"].'","'.$xml_data["total"].'","'.$xml_data["moneda"].'","'.$xml_data["sello"].'","'.$xml_data["rfc_emisor"].'","'.$xml_data["rfc_receptor"].'","'.$xml_data["uuid"].'","'.$xml_data["impuestos"].'","'.$codescded.'","'.$dasegg.'","'.$montopag.'","'.$saldoant.'","'.$id_genf.'")') or die ('<p>Error al registrar</p><br>' . mysqli_error($conexion));
...
$english_format_number = number_format($SUBBB=$tprec , 2, '.', ''); //subtotal cuenta
$ivaa=($SUBBB)*(.16); // iva

$english_format_number = number_format($totar=$ivaa+$tprec , 2, '.', ''); //TOTAL CUENTA

$sumadesc=0;

$english_format_number = number_format($sumadesc , 2, '.', ''); //sumadescuentos

$insertconc=mysqli_query($conexion,'INSERT INTO gen_concepto_fact(id_atencion,id_usua,codigo,cantidad,descripcion,codescded,iva,precio,importe_traslado,clave_unidad,unidad,obj_impuesto,fecha,tip_concepto,total) values ('.$id_at.','.$id_usua.',"'.$row['codigo'].'","'.$row['cantidad'].'","'.$row['descripcion'].'","'.$codescded.'","'.$ivaa.'","'.$row['importe'].'","'.$trasl.'","'. $row['clave_unidad'].'","'.$row['unidad'].'","'.$obj_impuestoo.'","'.$fecha_actual.'","Desglose","'.$totar.'")') or die ('<p>Error al registrar</p><br>' . mysqli_error($conexion));
...
<cfdi:Impuestos>   
  ...
  <cfdi:Impuestos TotalImpuestosTrasladados="'.$english_format_number = number_format($ivaa, 2, '.', '').'">
    <cfdi:Traslados>
      <cfdi:Traslado Base="'.$english_format_number = number_format($SUBBB, 2, '.', '').'" Impuesto="002" TipoFactor="Tasa" TasaOCuota="0.160000" Importe="'.$english_format_number = number_format($ivaa, 2, '.', '').'"/>
    </cfdi:Traslados>
  </cfdi:Impuestos>
</cfdi:Comprobante>';
$nnombre= "datos.xml";
...
// insert data
$saldoant=$xml_data["total"]-$montopag;
$insertconc=mysqli_query($conexion,'INSERT INTO comprobantes(id_atencion,id_usua,fecha,cadenaor,sellosat,sellocfd,nocertificado,version, subtotal, total, moneda, sello, rfc_emisor, rfc_receptor,uuid,iva,montopag,saldoant,externo,id_dat_gen_f) values ('.$id_at.','.$id_usua.',"'.$xml_data["fecha"].'","'.$cadenaOriginal.'","'.$xml_data["sellosat"] .'","'.$xml_data["cfd"].'","'.$xml_data["nocertificado"].'","'.$xml_data["version"].'","'.$xml_data["subtotal"].'","'.$xml_data["total"].'","'.$xml_data["moneda"].'","'.$xml_data["sello"].'","'.$xml_data["rfc_emisor"].'","'.$xml_data["rfc_receptor"].'","'.$xml_data["uuid"].'","'.$xml_data["impuestos"].'","'.$montopag.'","'.$saldoant.'","Si","'.$id_genf.'")') or die ('<p>Error al registrar</p><br>' . mysqli_error($conexion));
...
echo '<script type="text/javascript">window.location.href ="facturacion_serv.php?id_atencion='.$id_at.'" ;</script>';
  }
...
$insertconc=mysqli_query($conexion,'INSERT INTO gen_concepto_fact(id_atencion,id_usua,codigo,cantidad,descripcion,codescded,iva,precio,importe_traslado,clave_unidad,unidad,obj_impuesto,fecha,tip_concepto,total) values ('.$id_at.','.$id_usua.',"85101501","1","SERVICIOS HOSPITALARIOS DE EMERGENCIA O QUIRÚRGICOS","'.$codescdedl.'","'.$iva.'","'.$tprec.'","'.$trasl.'","'. $row['clave_unidad'].'","'.$row['unidad'].'","'.$obj_impuesto.'","'.$fecha_actual.'","Global","'.$tot.'")') or die ('<p>Error al registrar</p><br>' . mysqli_error($conexion));
...
$importeconceptos=($iim*.16);
 $cdesc=($sumadesc)*($iim);
        ...
        $xml .= '<cfdi:Concepto ClaveProdServ="85101501" Cantidad="1" ClaveUnidad="E48" Unidad="'.$unidad.'" Descripcion="SERVICIOS HOSPITALARIOS DE EMERGENCIA O QUIRÚRGICOS" ValorUnitario="'.$english_format_number = number_format($iim, 6, '.', '').'" Importe="'.$english_format_number = number_format($iim, 6, '.', '').'" Descuento="'.$english_format_number = number_format($sumadesc, 6, '.', '').'" ObjetoImp="'.$obj_impuesto.'">
...
$insertconc=mysqli_query($conexion,'INSERT INTO comprobantes(id_atencion,id_usua,fecha,cadenaor,sellosat,sellocfd,nocertificado,version, subtotal, total, moneda, sello, rfc_emisor, rfc_receptor,uuid,iva,descuento,descuento_aseg,montopag,saldoant,id_dat_gen_f) values ('.$id_at.','.$id_usua.',"'.$xml_data["fecha"].'","'.$cadenaOriginal.'","'.$xml_data["sellosat"] .'","'.$xml_data["cfd"].'","'.$xml_data["nocertificado"].'","'.$xml_data["version"].'","'.$xml_data["subtotal"].'","'.$xml_data["total"].'","'.$xml_data["moneda"].'","'.$xml_data["sello"].'","'.$xml_data["rfc_emisor"].'","'.$xml_data["rfc_receptor"].'","'.$xml_data["uuid"].'","'.$xml_data["impuestos"].'","'.$sumadesc.'","'.$codescdedl.'","'.$montopag.'","'.$saldoant.'","'.$id_genf.'")') or die ('<p>Error al registrar</p><br>' . mysqli_error($conexion));
...
$english_format_number = number_format($tprec=$SUBBIEN-$deposito , 2, '.', ''); //subtotal cuenta
...
$importeconceptos=($iim*.16);
 $cdesc=($sumadesc)*($iim);
        ...
        $xml .= '<cfdi:Concepto ClaveProdServ="85101501" Cantidad="1" ClaveUnidad="E48" Unidad="'.$row['unidad'].'" Descripcion="SERVICIOS HOSPITALARIOS DE EMERGENCIA O QUIRÚRGICOS" ValorUnitario="'.$english_format_number = number_format($CanInicial, 6, '.', '').'" Importe="'.$english_format_number = number_format($CanInicial, 6, '.', '').'" Descuento="'.$english_format_number = number_format($DesSiniva, 6, '.', '').'" ObjetoImp="'.$obj_impuesto.'">
        

<cfdi:Impuestos>   
        <cfdi:Traslados>
          <cfdi:Traslado Base="'.$english_format_number = number_format($bmiva, 6, '.', '').'" Impuesto="002" TipoFactor="Tasa" TasaOCuota="0.160000" Importe="'.$english_format_number = number_format($tcondes, 6, '.', '').'"/>
        </cfdi:Traslados>
      </cfdi:Impuestos>
...
$insertconc=mysqli_query($conexion,'INSERT INTO comprobantes(id_atencion,id_usua,fecha,cadenaor,sellosat,sellocfd,nocertificado,version, subtotal, total, moneda, sello, rfc_emisor, rfc_receptor,uuid,iva,descuento,montopag,saldoant,externo,id_dat_gen_f) values ('.$id_at.','.$id_usua.',"'.$xml_data["fecha"].'","'.$cadenaOriginal.'","'.$xml_data["sellosat"] .'","'.$xml_data["cfd"].'","'.$xml_data["nocertificado"].'","'.$xml_data["version"].'","'.$xml_data["subtotal"].'","'.$xml_data["total"].'","'.$xml_data["moneda"].'","'.$xml_data["sello"].'","'.$xml_data["rfc_emisor"].'","'.$xml_data["rfc_receptor"].'","'.$xml_data["uuid"].'","'.$xml_data["impuestos"].'","'.$DesSiniva.'","'.$montopag.'","'.$saldoant.'","Si","'.$id_genf.'")') or die ('<p>Error al registrar</p><br>' . mysqli_error($conexion));
