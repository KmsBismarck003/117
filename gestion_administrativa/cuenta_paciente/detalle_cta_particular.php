...
if(isset($id_encuesta) and $correo_encuesta == 'Si'){
 $id_encuesta = 'SI';
  }else{
    $id_encuesta ='NO';
    if ($alta_med=='SI' and ($correo_encuesta == 'NO' || $correo_encuesta == 'No')) {
     ?>
        <div class="col-md-12">
...
//       Se notifica alta del paciente: '.$pac_papell.' ' .$pac_sapell.' '.$pac_nom_pac.', para que se le realice su encuesta por favor.</h3></body></html>';
...
$sql_cart = "SELECT * FROM cart_almacen where paciente = $id_atencion and tipo = 'QUIROFANO' ORDER BY paciente";
$result_cart = $conexion->query($sql_cart);
...
$sql_cartf = "SELECT * FROM cart_almacen where paciente = $id_atencion and tipo = 'CONSULTA' ORDER BY paciente";
$result_cartf = $conexion->query($sql_cartf);
 
while ($row_cartf = $result_cartf->fetch_assoc()) {
  $cartq_idf = $row_cartf['id'];
}

if(isset($cartq_idf)){
         ?>
<div class="col-md-12">
            <div class="alert alert-danger alert-dismissible fade show"><center><strong>MEDICAMENTOS DE CONSULTA SIN CONFIRMAR POR FARMACIA</strong></center></div>
          </div>
  <?php }else{
        $cartq_idf ='nada';
    } ?>
          ...
          <?php
              ...
              while ($row3 = $resultado3->fetch_assoc()) {
               ...
               if ($insumo == 0 && $flag != 'S' && $flag != 'H' && 
                  ...
                  $resultado_prod = $conexion->query("SELECT * FROM item i where 
                    i.item_id = $insumo ") or die($conexion->error);
...
$fecha_actual = date("Y-m-d H:i:s");
        ...
        <div class="row">
          ...
          <div class="col-sm">
             <a href="excel_cta_particular.php?id_atencion=<?php echo $id_atencion ?>&id_usua=<?php echo $usuario1 ?>">
...
if ($diferencia >= 100000 and $correo == 'No'){
    $to="santiago@medicasanisidro.com";
    $subject='Control de cuentas de Medica "San Isidro"';
    $header='MIME-Version: 1.0'."\r\n";
    $from='https://sanisidro.simahospitales.com/';
    $header.="Content-type: text/html; charset=iso-8859-1\r\n";
    $header.="X-Mailer:PHP/".phpversion();
    $message.='<html><body>
    <h3 style="color:black;">Buen dia Santiago Uribe,</h3>';
    $message.='<h3 style="color:black;">
    Se notifica una diferencia en la cuenta del paciente: '.$pac_papell.' ' .$pac_sapell.' '.$pac_nom_pac.'
    , por un monto de $ ' . number_format($diferencia,2) .'  pesos.
    </h3></body></html>';
    $message=utf8_decode($message);
    if(mail($to,$subject,$message,$header)){ /* Condicion para recojer las variables y enviar el correo*/
        //  echo "Mensaje enviado correctamente";
    $sql_correo = "UPDATE dat_ingreso SET correo='Si' WHERE id_atencion = $id_atencion";
        $result_correo = $conexion->query($sql_correo);
    }else{
        //  echo "No se pudo enviar el email";
    }

/*$to2="guillermo@medicasanisidro.com";
$subject2='Control de cuentas de Medica "San Isidro"';
$header2='MIME-Version: 1.0'."\r\n";
$from2='https://sanisidro.simahospitales.com/';
$header2.="Content-type: text/html; charset=iso-8859-1\r\n";
$header2.="X-Mailer:PHP/".phpversion();
$message2.='<html><body>

<h3 style="color:black;">Buen dia Guillermo Carmona,</h3>';
$message2.='<h3 style="color:black;">
Se notifica una diferencia en la cuenta del paciente: '.$pac_papell.' ' .$pac_sapell.' '.$pac_nom_pac.'
, por un monto de $ ' . number_format($diferencia,2) .'  pesos.
</h3></body></html>';
 $message2=utf8_decode($message2);
