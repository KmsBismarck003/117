...
$sql2 = "SELECT * FROM stock_rojo1 WHERE item_id=$item_id";
...
$sql3 = "SELECT * FROM material_rojo1 i, item_type it WHERE material_id=$item_id and i.material_tipo=it.item_type_id";
$result_item = $conexion->query($sql3);
while ($row_item = $result_item->fetch_assoc()) {
   $item_id=$row_item['material_id'];
   $item_name=$row_item['material_nombre'];
   $item_code=$row_item['material_codigo'];
   $item_brand=$row_item['material_fabricante'];
   $item_grams=$row_item['material_contenido'];
   $item_type_desc=$row_item['item_type_desc'];
}

    $entrada= mysqli_query($conexion, 'INSERT INTO entradas_rojo1(item_id,entrada_qty,entrada_added,id_usua) 
   values ('.$item_id.','.$entradas.',"'.$fecha_actual.'",'.$id_usu.') ') or die('<p>Error al registrar</p><br>' . mysqli_error($conexion));

     if ($existe === "SI") {
         $total=$stock_qty+$qty;
         $totentra=$stock_entradas+$entradas;
         $sql1="UPDATE stock_rojo1 SET stock_qty=$total,stock_entradas=$totentra where stock_id=$stock_id";
         $result_stock=$conexion->query($sql1);
      }else{
    
         $stock= mysqli_query($conexion, 'INSERT INTO stock_rojo1(item_id,stock_qty,stock_entradas,stock_added,id_usua) 
