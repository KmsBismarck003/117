...
while($f = mysqli_fetch_array($resultado)){
     ...
     function Footer()
  {
    ...
    $this->Cell(0, 10, utf8_decode('CMSI-070'), 0, 1, 'R');
  }
  }
...
$pdf->Ln(7);
$pdf->MultiCell(131, 9, utf8_decode('SOLICITUD DE ESTUDIOS DE IMAGENOLOGÍA'), 0, 'C');

$sql_med_id = "SELECT * FROM notificaciones_imagen WHERE not_id=$notid and id_atencion=$id_atencion order by not_id limit 1";
...
$pdf->SetFont('Arial', 'B', 8);
$pdf->Cell(26, 6, utf8_decode('Médico tratante:'),0 , 0, 'L');
$pdf->SetFont('Arial', '', 8);
$pdf->Cell(108, 5.5, utf8_decode(' '.$user_prefijo.'. '.$user_tratante.' Cédula Prof. ' . $cedula), 'B', 'C');
$pdf->Ln(7.5);
$pdf->SetFont('Arial', 'B', 8);
...
$pdf->SetFont('Arial', 'B', 8);
$pdf->Cell(34, 3, utf8_decode("Detalle de estudio:"), 0, 'L');
$pdf->SetFont('Arial', '', 8);
$pdf->MultiCell(100, 3, utf8_decode($observa),'B', 'J');


$d="";
    $sql_mot = "SELECT motivo_atn from dat_ingreso where id_atencion=$id_atencion ORDER by motivo_atn DESC LIMIT 1";
    $result_mot = $conexion->query($sql_mot);
    while ($row_mot = $result_mot->fetch_assoc()) {
        $d=$row_mot['motivo_atn'];
    } 
...
$pdf->Ln(3);
  ...
  $sql_med_id = "SELECT id_usua FROM notificaciones_imagen WHERE not_id=$notid and id_atencion=$id_atencion order by not_id limit 1";
      $result_med_id = $conexion->query($sql_med_id);
      ...
      $pdf->Image('../../imgfirma/' . $firma, 65, 70, 15);
