...
} else if ($usuario['id_rol'] == 4 || $usuario['id_rol'] == 5 || $usuario['id_rol'] == 12) {
    include "../header_labo.php";
} else{
    //session_unset();
    // session_destroy();
...
<div class="thead" style="background-color: #2b2d7f ; color: white; font-size: 25px;">
         <tr><strong><center>RESULTADOS DE ESTUDIOS DE LABORATORIO</center></strong>
      </div><br>
        ...
        <div class="content">
            ...
            <div class="table-responsive">
                ...
                <table class="table table-bordered table-striped" id="mytable">
                    ...
                    $query = "SELECT * FROM notificaciones_labo n, reg_usuarios u where n.realizado = 'SI' and n.id_usua = u.id_usua  order by fecha_resul DESC";
                    ...
                    while ($row_pac = $result_pac->fetch_assoc()) {
                        ...
                        echo '<tr>'
                            ...
                            . '<td >' . $medico . '</td>'
                            . '<td ><a style="color: deepskyblue;>
                            <span class="glyphicon glyphicon-eye-open" aria-hidden="true"></span> ' . $row['sol_estudios'] .'/'. $row['det_labo'] . '</a></td>'
                              . '<td class="fondo" style="color:white;">
                                    <center><a href="../Laboratorio/pdf_solicitud_estu.php?id_atencion='.$row['id_atencion'].'&notid='.$row['not_id'].'&medico='.$row['papell'] .'&paciente='.$pac.'&tipo='.$row['sol_estudios'].'" target="_blank" ><button type="button" class="btn btn-success"><i class="fa fa-file-pdf-o" aria-hidden="true"></i></button></td></center>';
                            
                            echo '<td class="fondo" style="color:white;"><center>'
                            . ' <a href="../Laboratorio/editar_labo.php?not_id=' . $row['not_id'] . '" title="Editar resultados" class="btn btn-danger "><span class="fa fa-edit" aria-hidden="true"></span></a>';

                            echo '<td class="fondo" style="color:white;"><center>'
                            . ' <a href="../Laboratorio/verpdf.php?not_id=' . $row['not_id'] . '" title="Ver resultados" class="btn btn-danger "><span class="fa fa-file-pdf-o" aria-hidden="true"></span></a>';
                            echo '</center></td>';
                        ...
                        if($usuario['id_rol'] == 5){
                       echo '<td class="fondo" style="color:white;"><center>'
                            . ' <a href="el_est.php?not_id=' . $row['not_id'] . '" title="Eliminar estudio" class="btn btn-warning "><span class="fa fa-trash" aria-hidden="true"></span></a>';
                            echo '</center></td>';
                         } 
                           ...
                           . '<td >' . $resp .  '</td></tr>';
                            ...
                            . '<td >' . date_format($fecha_orden,"d/m/Y H:i a") . '</td>'
                            . '<td >' . $medico . '</td>'
                            . '<td ><a style="color: deepskyblue;" href="../Laboratorio/ver_paquete.php?nom=' . $row['sol_estudios'] . '"><span class="glyphicon glyphicon-eye-open" aria-hidden="true"></span> ' . $row['sol_estudios'] . '</a></td>'
                            . '<td >' . $row['realizado'] . '</td>';
