...
$pdf->Ln(7);
$pdf->MultiCell(131, 9, utf8_decode('SOLICITUD DE PRODUCTOS SANGUÍNEOS'), 0, 'C');

$sql_med_id = "SELECT * FROM notificaciones_sangre WHERE not_id=$notid and id_atencion=$id_atencion order by not_id limit 1";
      ...
      while ($row_med_id = $result_med_id->fetch_assoc()) {
        ...
        $observa = $row_med_id['det_sang'];
        $tipo = $row_med_id['sol_sangre'];
...
$pdf->Cell(95, 4, utf8_decode($id_exp.' - '.$paciente),'B', 'C');
$pdf->SetFont('Arial', 'B', 8);
...
$pdf->SetFont('Arial', 'B', 8);
$pdf->Cell(9, 3, utf8_decode("Edad:"), 0, 0, 'L');
$pdf->SetFont('Arial', '', 8);
if($anos > "0" ){
  $pdf->SetFont('Arial', 'B', 8);
$pdf->Cell(18, 3, utf8_decode($anos . ' años' ), 'B', 'C');
}elseif($anos <="0" && $meses>"0"){
     $pdf->SetFont('Arial', 'B', 8);
$pdf->Cell(18, 3, utf8_decode($meses), 'B', 'C');
}elseif($anos <="0" && $meses<="0" && $dias>"0"){
    $pdf->SetFont('Arial', 'B', 8);
$pdf->Cell(18, 3, utf8_decode($dias), 'B', 'C');
}
$pdf->SetFont('Arial', 'B', 8);
$pdf->Cell(11, 3, utf8_decode(" Sexo:"), 0, 0, 'C');
$pdf->SetFont('Arial', '', 8);
$pdf->Cell(-1, 3, utf8_decode(""), 0, 0, 'C');
$pdf->Cell(13, 3, utf8_decode($sexo),'B', 'C');

$pdf->SetFont('Arial', 'B', 8);
$pdf->Cell(25, 3, utf8_decode(" Fec. nacimiento:"), 0, 0, 'C');
$fechaes=date_create($fecnac);
$pdf->SetFont('Arial', '', 8);
$pdf->Cell(16, 3, date_format($fechaes,'d/m/Y'),'B', 'C');

$pdf->SetFont('Arial', 'B', 8);
$pdf->Cell(25, 3, utf8_decode(" Tipo de sangre:"), 0, 0, 'C');
$pdf->SetFont('Arial', '', 8);
$pdf->Cell(19, 3, $tip_san,'B', 'C');
$pdf->Ln(4);
...
$pdf->SetFont('Arial', 'B', 8);
$pdf->Cell(34, 3, utf8_decode("Producto(s) solicitado(s):"), 0, 'L');
$pdf->SetFont('Arial', '', 8);
$pdf->MultiCell(100, 3, utf8_decode($tipo . ' ' . $tipins),'B', 'J');
$pdf->SetFont('Arial', 'B', 8);
$pdf->Cell(34, 4, utf8_decode("Detalle:"), 0, 'L');
$pdf->SetFont('Arial', '', 8);
$pdf->MultiCell(100, 4, utf8_decode($observa),'B', 'J');


$d="";
    $sql_mot = "SELECT diag_paciente from diag_pac where Id_exp=$id_atencion ORDER by diag_paciente DESC LIMIT 1";
    $result_mot = $conexion->query($sql_mot);
    while ($row_mot = $result_mot->fetch_assoc()) {
        $d=$row_mot['diag_paciente'];
    } 
...
if ($d!=null) {
    $pdf->SetFont('Arial', 'B', 8);
    $pdf->Cell(34, 4, utf8_decode('Diagnóstico probable:') , 0, 'C');
    $pdf->SetFont('Arial', '', 7.5);
    $pdf->MultiCell(100, 4, utf8_decode($d) , 'B', 'J');
    }

$pdf->Ln(3);


  $sql_med_id = "SELECT id_usua FROM notificaciones_sangre WHERE not_id=$notid and id_atencion=$id_atencion order by not_id limit 1";
      $result_med_id = $conexion->query($sql_med_id);
