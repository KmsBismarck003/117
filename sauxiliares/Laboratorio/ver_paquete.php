...
<div class="container-fluid">
    ...
    if ($usuario1['id_rol'] == 4) {
        ...
        ?>
        <a type="submit" class="btn btn-primary btn-block col-1" href="../../template/menu_gerencia.php">REGRESAR</a>
        ...
        <div class="col  col-12">
            <h2>
                <a href="" data-target="#sidebar" data-toggle="collapse" class="d-md-none"><i class="fa fa-bars"
                                                                                              id="side"></i></a>
                <center><font id="letra"><i class="fa fa-plus-square"></i> Solicitud de Estudios: <?php echo  $nombre_paq ?></font>
            </h2>
            </center>
            <hr><center>
            <a type="submit" class="btn btn-primary btn-block col-1" href="insert_paquete_labo.php">NUEVO KIT</a></center>
        ...
        //    $resultado2 = $conexion->query("SELECT id_serv, serv_cve, serv_costo, serv_umed,serv_activo FROM cat_servicios") or die($conexion->error);
            ...
            <div class="table-responsive">
                ...
                <table class="table table-bordered table-striped" id="mytable">
                    ...
                    <tbody>
                    <?php
                    $sql = 'SELECT serv_cve, serv_desc , cantidad, estudio_id,clave,id_serv,id_paquete FROM paquetes_labo p, cat_servicios m where m.id_serv=p.estudio_id and p.nombre = "'.$nombre_paq.'"';
                 ...
                 //   $resultado2 = $conexion->query('SELECT  material_nombre, cantidad FROM paquetes_ceye p, material_ceye m where m.material_id=p.material_id and p.nombre = "$nombre_paq"') or die($conexion->error);
                    ...
                    while ($row = $result->fetch_assoc()) {
                        ...
                        ?>
                        <tr>
                            <td> <?php echo $row['serv_cve'] ?></td>
                            <td> <?php echo $row['serv_desc'] ?></td>
                            <td> <?php echo $row['cantidad'] ?></td>
                             <td><a role="button" class="btn btn-block btn-danger col-sm-4" data-toggle="modal" data-target="#exampleModal2<?php echo $id_serv;?>"><font color="white">X</font></a></td>
                         </tr>

                         
                       <?php 
                       $no++;
include 'eliminar_estudio.php';
include 'agregar_estudio.php';

                    }
                    ?>
                    <a role="button" class="btn btn-block btn-success col-sm-4" data-toggle="modal" data-target="#exampleModal1<?php echo $clave;?>"><font color="white">AGREGAR ESTUDIOS AL KIT <i class="fa fa-plus-square-o" aria-hidden="true"></i></font></a>
                    </tbody>
                </table>
