...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($coinciden) {
        ...
        if (isset($_POST['enviar_datos'])) {
            ...
            $query_carrito = "SELECT entrada_fecha, id_compra, item_id, entrada_lote, entrada_caducidad, entrada_qty, entrada_unidosis, entrada_costo, ubicacion_id FROM carrito_entradasq WHERE id_compra = ?";
            ...
            if ($result_carrito->num_rows > 0) {
                while ($row = $result_carrito->fetch_assoc()) {
                    ...
                    $query_entrada = "INSERT INTO entradas_almacenq (
                ...
                $query_eliminar_carrito = "DELETE FROM carrito_entradasq WHERE id_compra = ?";
...
if (!empty($updates)) {
    ...
    $sql = "UPDATE carrito_entradasq SET " . implode(", ", $updates) . " WHERE id_compra = ? AND item_id = ?";
...
if (isset($_GET['id_compra'])) {
    ...
    try {
        ...
        $sql1 = "DELETE FROM carrito_entradasq WHERE id_compra = ?";
...
$query_total = "SELECT COUNT(DISTINCT id_compra) AS total FROM carrito_entradasq";
$result_total = $conexion->query($query_total);
$total_registros = $result_total->fetch_assoc()['total'];
$total_paginas = ceil($total_registros / $registros_por_pagina);

// Consulta para obtener los IDs de las órdenes con paginación
$query_ordenes = "SELECT DISTINCT id_compra FROM carrito_entradasq LIMIT $offset, $registros_por_pagina";
