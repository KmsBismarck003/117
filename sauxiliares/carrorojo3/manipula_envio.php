...
/*$sql2 = "INSERT INTO cart_recib(item_id,cart_qty,destino,cart_uniqid,almacen)VALUES($item_id,$qty,'TOLUCA','$cart_uniquid','FARMACIA');";
 ...
 $ingresar2 = mysqli_query($conexion_almacen, 'INSERT INTO cart_recib(item_id,cart_qty,destino,cart_uniqid,almacen) values ('.$item_id.',' . $qty . ',"TOLUCA","' . $cart_uniquid . '","CARRO ROJO3") ') or die('<p>Error al registrar</p><br>' . mysqli_error($conexion_almacen));
...
$sales= mysqli_query($conexion_almacen, 'INSERT INTO sales_almacen(item_id,qty,destino,almacen,stock_lote,sales_expiry,id_usua) 
   values ('.$item_id.','.$env_qty.',"TOLUCA","CARRO ROJO3","'.$lote.'","'.$cad.'",'.$id_usu.') ') or die('<p>Error al registrar</p><br>' . mysqli_error($conexion_almacen));
