  ...
  <section class="content container-fluid">
            ...
            <div class="form-group">
                ...
                <select class="form-control col-sm-3"  data-live-search="true" name="paciente" required>
                    ...
                    while ($row = $result->fetch_assoc()) {
                        ...
                        $area= $row['area'];
                        if ($area == 'ALTA'|| $area =='TRIAGE'){$area = 'OBSERVACIÓN';}
                        $cama = " ";
    ...
    if ((isset($_GET['paciente']))) {
      ...
      while ($row_pac = $result_pac->fetch_assoc()) {
        $area =  $row_pac['area']; 
        if ($area == 'ALTA'|| $area =='TRIAGE'||$area == 'OBSERVACIÓN'){$area = 'OBSERVACION';}
        else {$area = 'QUIROFANO';}
...
<option value="">SELECCIONA UN MEDICAMENTO</option>
    ...
    if (isset($_POST['btnserv'])) {
      ...
      if (!($stock < $stock_min)) {
        ...
        $sql2 = "INSERT INTO cart_almacen(item_id,cart_surtido,cart_stock_id,id_usua,cart_uniqid, paciente,enf_fecha,cart_hora,cart_fecha,medicam_mat,tipo,cart_price)VALUES
        ($item_id,$qty, $stock_id,$usuario2,'$cart_uniquid', $paciente1,'$fecha','$hora','$fecha_reg','$medicam_mat','$area',$precio)";
          ...
          <?php } ?>
            <div class="col-md-12">
              ...
              <center>
                <?php
                if ($area!='QUIROFANO') {
                echo '<a type="submit" class="btn btn-success col-3 btn-block" href="manipula_carnew.php?q=comf_cart&paciente=' . $paciente1 . '&id_usua=' . $usuario2 . '"><span>CONFIRMAR INSUMOS</span></a>';
                }?>
              </center>
            </div>
          </div>
