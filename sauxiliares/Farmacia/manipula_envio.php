...
/*$sql2 = "INSERT INTO cart_recib(item_id,cart_qty,destino,cart_uniqid,almacen)VALUES($item_id,$qty,'TOLUCA','$cart_uniquid','FARMACIA');";
 ...
 $ingresar2 = mysqli_query($conexion_almacen, 'INSERT INTO cart_recib(item_id,cart_qty,destino,cart_uniqid,almacen) values ('.$item_id.',' . $qty . ',"TOLUCA","' . $cart_uniquid . '","FARMACIA") ') or die('<p>Error al registrar</p><br>' . mysqli_error($conexion_almacen));
...
$result_cart = $conexion_almacen->query($sql3);

$total=$stock_qty-$env_qty;
$salidas=$stock_salidas+$env_qty;
/*
$sql4 = "UPDATE stock_almacen SET stock_qty=$total, stock_salidas=$salidas WHERE stock_lote='$lote'";
$result_cart = $conexion_almacen->query($sql4);
*/
$sales= mysqli_query($conexion_almacen, 'INSERT INTO sales_almacen(item_id,qty,destino,almacen,stock_lote,sales_expiry,id_usua) 
   values ('.$item_id.','.$env_qty.',"TOLUCA","FARMACIA","'.$lote.'","'.$cad.'",'.$id_usu.') ') or die('<p>Error al registrar</p><br>' . mysqli_error($conexion_almacen));
   ...
   echo '<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/sweetalert/1.1.3/sweetalert.css">';
    ...
    echo '<script>
                    $(document).ready(function() {
                        swal({
                            title: "NO HAY EXISTENCIAS DE ESTE MEDICAMENTO", 
                            type: "error",
