...
$sql = "SELECT * FROM existencias_almacenh WHERE item_id = $item_idi and existe_lote = $lote and existe_caducidad = $caducidad";
...
if ($row_existencias_almacenh = $result_existencias_almacenh->fetch_assoc()) {
    ...
    // Validar si el dev_qty es igual al valor devuelto previamente
    if ($dev_qtyi == $existe_devoluciones) {
        // Solo sumar la cantidad devuelta
        $total = $existe_qty + $cant_invi;
        $totdevol = $existe_devoluciones + $cant_invi;

        // Actualizar la tabla con los nuevos valores
        $sql1 = "UPDATE existencias_almacenh
                SET existe_qty = $total, 
                    existe_devoluciones = $totdevol, 
                    existe_lote = '$lote',
                    existe_caducidad = '$caducidad'
                WHERE existe_id = $existe_id";
        $result_existencias_almacenh = $conexion->query($sql1);
    } else {
        // Mostrar un error si dev_qty no coincide
        die('Error: La cantidad devuelta no coincide con el registro existente.');
    }
} else {
    // Si no existe el registro, insertar uno nuevo
    $existencias_almacenh = mysqli_query(
        $conexion,
        "INSERT INTO existencias_almacenh(item_id, existe_qty, existe_devoluciones, existe_fecha, id_usua, existe_lote, existe_caducidad) 
        VALUES ($item_idi, $cant_invi, $cant_invi, '$fecha_actual', $id_usua, '$lote', '$caducidad')"
    ) or die('<p>Error al registrar en existencias_almacenh</p>' . mysqli_error($conexion));
}

// Obtener la cantidad disponible actual en existencias_almacenh (existe_qty)
$sql_existencias = "SELECT existe_qty FROM existencias_almacenh WHERE item_id = $item_idi";
...
if ($row_existencias = $result_existencias->fetch_assoc()) {
    ...
    die('Error al obtener las existencias de existencias_almacenh: ' . mysqli_error($conexion));
}
...
$sql2 = "UPDATE devoluciones_almacenh 
...
if ($new_qty === 0) {
    $sql3 = "UPDATE devoluciones_almacenh 
            SET dev_estatus = 'NO' 
            WHERE dev_id = $id_devi";
...
$sql_kardex = "INSERT INTO kardex_almacenh (item_id, kardex_fecha, kardex_lote, kardex_caducidad, kardex_inicial, kardex_entradas, kardex_salidas, kardex_qty, kardex_dev_stock, kardex_dev_merma, kardex_movimiento, kardex_ubicacion, kardex_destino, id_usua
...
// Ejecutar el INSERT en la tabla kardex_almacenh
$result_kardex = $conexion->query($sql_kardex);

if (!$result_kardex) {
    die('Error al registrar en kardex_almacenh: ' . mysqli_error($conexion));
}

// Redirigir al usuario
echo '<script type="text/javascript"> window.location.href="devolucionesh.php";</script>';
?>
