...
error_reporting(E_ALL & ~E_DEPRECATED); // Oculta advertencias por funciones deprecated
...
$pdf->Ln(5);
$pdf->Cell(95, 5, utf8_decode('Alergias: ' . $paciente['alergias']), 1);
$pdf->Cell(95, 5, utf8_decode('Diagnóstico: ' . $paciente['motivo_atn']), 1);
$pdf->Ln(5);
$ingreso = date_create($paciente['fecha']);
$pdf->Cell(190, 5, 'Fecha de Ingreso: ' . date_format($ingreso, 'd-m-Y H:i a'), 1);
$pdf->Ln(7);
...
$stmt = $conexion->prepare("SELECT medicamento, dosis, intervalo, horario, via_administracion, ultima_dosis, continuidad, id_usua FROM trat_farma WHERE id_atencion = ?");
$stmt->bind_param("i", $id_atencion);
$stmt->execute();
...
$pdf->SetFont('Arial', 'B', 12);
$pdf->Cell(0, 10, utf8_decode('Medicamentos surtidos por Farmacia'), 0, 1, 'L');
$pdf->SetFont('Arial', '', 8);
...
$pdf->SetFont('Arial', 'B', 12);
$pdf->Cell(0, 10, utf8_decode('Perfil Farmacológico'), 0, 1, 'L');
$pdf->SetFont('Arial', 'B', 6);
$pdf->SetFillColor(217, 217, 217);

$pdf->Cell(90, 5, utf8_decode('Medicamentos'), 1, 0, 'C', true);
$pdf->Cell(20, 5, utf8_decode('Dosis'), 1, 0, 'C', true);
$pdf->Cell(19, 5, utf8_decode('Vía'), 1, 0, 'C', true);
$pdf->Cell(17, 5, utf8_decode('Fecha'), 1, 0, 'C', true);
$pdf->Cell(10, 5, utf8_decode('Hora'), 1, 0, 'C', true);
$pdf->Cell(39, 5, utf8_decode('Turno'), 1, 0, 'C', true);
...
while ($cis_s = $cis->fetch_assoc()) {
    ...
    $pdf->Cell(90,5, utf8_decode($cis_s['medicam_mat']),1,0,'C');
    $pdf->Cell(20,5, utf8_decode($cis_s['dosis_mat'].' ML'),1,0,'C');
    $pdf->Cell(19,5, utf8_decode($cis_s['via_mat']),1,0,'C');
    $pdf->Cell(17,5, date_format($date,"d/m/Y"),1,0,'C');
    $pdf->Cell(10,5, utf8_decode($cis_s['hora_mat']),1,0,'C');
    $pdf->Cell(39,5, utf8_decode($cis_s['turno']),1,0,'C');
}
$pdf->Ln(30);

// Firma del profesional
if ($id_usua !== null) {
    $stmt = $conexion->prepare("SELECT * FROM reg_usuarios WHERE id_usua = ?");
    $stmt->bind_param("i", $id_usua);
    $stmt->execute();
    $result_reg_usrs = $stmt->get_result();

    if ($row = $result_reg_usrs->fetch_assoc()) {
        $pre = $row['pre'];
        $app = $row['papell'];
        $apm = $row['sapell'];
        $nom = $row['nombre'];
        ...
        $pdf->Image('../../imgfirma/' . $firma, 40, 246, 30);
        $pdf->SetY(260);
        $pdf->SetFont('Arial', '', 8);
        $pdf->Cell(95, 5, utf8_decode($pre . '. ' . $app . ' ' . $apm . ' ' . $nom), 0, 0, 'C');
        $pdf->Cell(95, 5, '_______________________________', 0, 0, 'C');
        $pdf->Cell(95, 5, '_______________________________', 0, 1, 'C');
        $pdf->Cell(95, 5, utf8_decode('Nombre y Firma del Profesional Farmacéutico'), 0, 0, 'C');
        $pdf->Cell(95, 5, utf8_decode('Nombre y Firma del Paciente o Familiar'), 0, 1, 'C');
    }
}
