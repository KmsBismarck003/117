...
if ($merma_qty > 0) {
    // Prepara la consulta SQL para insertar los datos en la tabla merma_almacenh
    $query_merma = "INSERT INTO merma_almacenh (item_id, merma_lote, merma_caducidad, merma_qty, merma_motivo, id_usua)
                    VALUES (?, ?, ?, ?, ?, ?)";
    ...
    if ($stmt_merma = $conexion->prepare($query_merma)) {
        ...
        if ($stmt_merma->execute()) {
            // Ahora, actualiza el kardex_almacenh
            // Primero, obtiene la cantidad actual del producto en el kardex
            $query_kardex = "SELECT kardex_qty FROM kardex_almacenh WHERE item_id = ? AND kardex_lote = ? AND kardex_caducidad = ? ORDER BY kardex_fecha DESC LIMIT 1";
            ...
            if ($stmt_kardex = $conexion->prepare($query_kardex)) {
                ...
                $query_kardex_insert = "INSERT INTO kardex_almacenh (item_id, kardex_lote, kardex_caducidad, kardex_qty, kardex_dev_merma, kardex_movimiento, id_usua, kardex_ubicacion, kardex_destino)
                                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
