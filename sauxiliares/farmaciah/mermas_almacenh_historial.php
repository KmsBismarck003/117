...
if (isset($_POST['inicial']) && isset($_POST['final'])) {
    ...
    // Consulta para obtener los datos de la tabla `merma_almacenh` con JOIN y filtro de fechas
...
<head>
    ...
    <title>Historial de Mermas - Farmacia Hospitalaria</title>
...
<body>
        ...
        <div class="container-moderno">
            ...
            <div class="header-principal">
                <div class="contenido-header">
                    <i class="fas fa-exclamation-triangle icono-principal"></i>
                    <h1>HISTORIAL DE MERMAS - FARMACIA HOSPITALARIA</h1>
                </div>
            </div>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <input type="date" class="form-control" name="inicial" 
                                       value="<?= isset($_POST['inicial']) ? $_POST['inicial'] : date('Y-m-01') ?>">
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <input type="date" class="form-control" name="final" 
                                       value="<?= isset($_POST['final']) ? $_POST['final'] : date('Y-m-d') ?>">
                            </div>
                        </div>
                        <div class="col-md-4">
                    ...
                    <table class="tabla-moderna table table-striped">
                        ...
                        <tbody>
                            ...
                            while ($row = $resultado->fetch_assoc()): 
                                $total_merma += $row['merma_qty'];
                                
                                // Contar motivos
                                $motivo = $row['merma_motivo'];
                                if (isset($motivos_count[$motivo])) {
                                    $motivos_count[$motivo]++;
                                } else {
                                    $motivos_count[$motivo] = 1;
                                }
                                
                                // Determinar clase de badge para cantidad según la cantidad
                                $cantidad = $row['merma_qty'];
                                $clase_cantidad = '';
                                if ($cantidad > 100) {
                                    $clase_cantidad = 'badge-critica';
                                } elseif ($cantidad > 50) {
                                    $clase_cantidad = 'badge-moderada';
                                } else {
                                    $clase_cantidad = 'badge-leve';
                                }
                            ?>
                                <tr>
                                    <td>
                                        <span class="badge badge-fecha"><?= $row['merma_id'] ?></span>
                                    </td>
                                    <td>
                                        <span class="badge badge-fecha">
                                            <?= date('d/m/Y', strtotime($row['merma_fecha'])) ?>
                                        </span>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <span class="badge badge-lote">
                                            <?= htmlspecialchars($row['merma_lote']) ?>
                                        </span>
                                    </td>
                                    <td>
                                        <span class="badge badge-caducidad">
                                            <?= date('d/m/Y', strtotime($row['merma_caducidad'])) ?>
                                        </span>
                                    </td>
                                    <td>
                                        <span class="badge-merma <?= $clase_cantidad ?>">
                                            <i class="fas fa-times-circle"></i> <?= number_format($row['merma_qty']) ?>
                                        </span>
                                    </td>
                                    <td>
                                        <span class="badge badge-warning">
                                            <i class="fas fa-exclamation-triangle"></i>
                                            <?= htmlspecialchars($row['merma_motivo']) ?>
                                        </span>
                                    </td>
                                    <td>
                    ...
                    <div class="col-md-4">
                        <div class="card-estadistica">
                            <div class="numero">
                                ...
                                <?= number_format($total_merma) ?>
                            </div>
                            <div class="descripcion">Total de Unidades en Merma</div>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="card-estadistica">
                            ...
                            <div class="descripcion">Total de Registros de Merma</div>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="card-estadistica">
                            ...
                            <div class="descripcion">Tipos de Motivos Diferentes</div>
                        </div>
                    </div>
                </div>

                <!-- Desglose por motivos -->
                <?php if (count($motivos_count) > 0): ?>
                <div class="contenedor-filtros mt-4">
                    <div class="titulo-filtros">
                        <i class="fas fa-chart-pie"></i> Desglose por Motivos de Merma
                    </div>
                    <div class="row">
                        <?php foreach ($motivos_count as $motivo => $cantidad): ?>
                        <div class="col-md-6 col-lg-4 mb-3">
                            <div class="alert alert-warning">
                                <strong><i class="fas fa-exclamation-triangle"></i> <?= htmlspecialchars($motivo) ?></strong>
                                <br>
                                <small><?= $cantidad ?> registro<?= $cantidad > 1 ? 's' : '' ?></small>
                            </div>
                        </div>
                        <?php endforeach; ?>
                    </div>
            ...
            <?php else: ?>
                <div class="tabla-contenedor">
                    <div class="mensaje-sin-resultados">
                        ...
                        <p>No se encontraron registros de mermas.</p>
    ...
    <script>
        $(document).ready(function() {
            ...
            function agregarBusqueda() {
                if ($('.tabla-moderna tbody tr').length > 10) {
                            ...
                            <div class="col-md-6">
                                <div class="input-group">
                                    ...
                                    <input type="text" id="buscarTabla" class="form-control" 
                                           placeholder="Buscar medicamento, lote, motivo...">
                                </div>
                            </div>
                            <div class="col-md-6">
                    ...
                    $('.tabla-moderna tbody tr').each(function() {
                        var motivo = $(this).find('td:nth-child(8)').text().trim();
                        if (motivo && motivos.indexOf(motivo) === -1) {
                            motivos.push(motivo);
                        }
                    });
                    
                    // Agregar opciones al select de motivos
                    motivos.forEach(function(motivo) {
                        $('#filtroMotivo').append('<option value="' + motivo + '">' + motivo + '</option>');
                    });
                    
                    // Función de búsqueda por texto
                    $('#buscarTabla').on('keyup', function() {
                        var value = $(this).val().toLowerCase();
                        var motivoSeleccionado = $('#filtroMotivo').val();
                        
                        $('.tabla-moderna tbody tr').filter(function() {
                            var textoFila = $(this).text().toLowerCase();
                            var motivoFila = $(this).find('td:nth-child(8)').text().trim();
                            
                            var coincideTexto = textoFila.indexOf(value) > -1;
                            var coincideMotivo = motivoSeleccionado === '' || motivoFila === motivoSeleccionado;
                            
                            $(this).toggle(coincideTexto && coincideMotivo);
                        });
                    });
                    
                    // Función de filtro por motivo
                    $('#filtroMotivo').on('change', function() {
                        var motivoSeleccionado = $(this).val();
                        var textoIngresado = $('#buscarTabla').val().toLowerCase();
                        
                        $('.tabla-moderna tbody tr').filter(function() {
                            var textoFila = $(this).text().toLowerCase();
                            var motivoFila = $(this).find('td:nth-child(8)').text().trim();
                            
                            var coincideTexto = textoIngresado === '' || textoFila.indexOf(textoIngresado) > -1;
                            var coincideMotivo = motivoSeleccionado === '' || motivoFila === motivoSeleccionado;
            ...
            // Función para resaltar mermas críticas
            function resaltarMermasCriticas() {
                $('.tabla-moderna tbody tr').each(function() {
                    var cantidadText = $(this).find('.badge-merma').text().replace(/\D/g, '');
                    var cantidad = parseInt(cantidadText);
                    
                    if (cantidad > 100) {
                        $(this).addClass('merma-critica');
                    } else if (cantidad > 50) {
                        $(this).addClass('merma-moderada');
                    }
                });
            }
            
            // Aplicar resaltado de mermas críticas
            resaltarMermasCriticas();
            ...
            if ($('.tabla-moderna tbody tr').length > 0) {
                var exportButton = `
                    <div class="text-right mb-3">
                        <button class="btn-moderno" style="background: linear-gradient(135deg, #17a2b8 0%, #138496 100%); color: white;" onclick="exportarDatos()">
                            <i class="fas fa-download"></i> Exportar Datos
                        </button>
                    </div>
                `;
                $('.tabla-contenedor').before(exportButton);
            }
        });
        
        // Función para exportar datos (simulada)
        function exportarDatos() {
            alert('Función de exportación en desarrollo. Los datos se exportarían a Excel o PDF.');
        }
    </script>
    ...
    <style>
        ...
        /* Estilos para los badges de cantidad según el nivel */
        .badge-critica {
